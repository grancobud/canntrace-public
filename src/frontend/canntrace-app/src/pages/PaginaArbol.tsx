...
const ETAPA_STYLE: Record<EtapaTone, { bg: string; border: string; accent: string; label: string }> = {
  pm:      { bg: 'rgba(196,154,44,0.06)',  border: '#3a2f15', accent: '#E3B94A', label: 'PLANTA MADRE' },
  esq:     { bg: 'rgba(63,176,116,0.06)',  border: '#1f3a26', accent: '#6bcf8e', label: 'ESQUEJADO' },
  veg:     { bg: 'rgba(63,176,116,0.10)',  border: '#2a5138', accent: '#8fe0a8', label: 'VEGETATIVA' },
  flo:     { bg: 'rgba(196,154,44,0.10)',  border: '#5a4820', accent: '#E3B94A', label: 'FLORACIÓN' },
  cos:     { bg: 'rgba(180,200,190,0.06)', border: '#243429', accent: '#a7b1ab', label: 'COSECHA' },
  ...
  cuar:    { bg: 'rgba(196,154,44,0.06)',  border: '#3a2f15', accent: '#C49A2C', label: 'CUARENTENA' },
  frac:    { bg: 'rgba(63,176,116,0.06)',  border: '#1f3a26', accent: '#6bcf8e', label: 'FRACCIONAMIENTO' },
  dep:     { bg: 'rgba(196,154,44,0.12)',  border: '#5a4820', accent: '#E3B94A', label: 'DEPÓSITO FINAL' },
  default: { bg: 'rgba(180,200,190,0.05)', border: '#1a2620', accent: '#8e9892', label: 'LOTE' },
}
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2 space-y-1">
        <div className="text-[11px] font-semibold tabular-nums leading-tight" style={{ color: '#e8ece9', wordBreak: 'break-all' }}>
          {data.codigo_lote}
        </div>
        ...
        {peso && (
          <div className="text-[9.5px]" style={{ color: '#a7b1ab' }}>{peso}</div>
        )}
      </div>
    </div>
  )
...
function ArbolFlow({ nodes, edges }: { nodes: Node[]; edges: Edge[] }) {
  ...
  return (
    <div className="h-full w-full" style={{ background: '#0a0f0d' }}>
      <ReactFlow
      ...
      >
        ...
        <Controls
          ...
          className="!bg-[#0e1411] !border !border-[#1a2620] !rounded-lg !shadow-none [&>button]:!bg-transparent [&>button]:!border-b [&>button]:!border-[#1a2620] [&>button:last-child]:!border-b-0 [&>button]:!text-[#8fe0a8] [&>button:hover]:!bg-[#3fb074]/10"
        />
      </ReactFlow>
      <button
        onClick={() => fitView({ padding: 0.25, duration: 400 })}
        className="absolute top-4 right-4 z-10 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-[#3fb074]/40 bg-[#0e1411]/90 hover:bg-[#3fb074]/15 text-[11px] font-medium text-[#8fe0a8]"
        title="Ajustar vista"
      >
...
export default function PaginaArbol() {
  ...
  const { nodes, edges, etapasUsadas, totalLotes } = useMemo(() => {
    ...
    const es: Edge[] = (data.aristas || []).filter((a: any) => a.origen && a.destino).map((a: any, i: number) => ({
      ...
      style: { stroke: '#3fb074', strokeWidth: 1, opacity: 0.25 },
      markerEnd: { type: MarkerType.ArrowClosed, color: '#3fb074', width: 12, height: 12 },
    }))
  ...
  return (
    <div className="flex-1 overflow-hidden flex flex-col bg-[#0a0f0d] text-[#d3d8d4] font-sans">
      {/* TopBar */}
      <div className="bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620] flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <div className="min-w-0 flex items-center gap-2.5">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-[#3fb074]/10 border border-[#3fb074]/20 shrink-0">
              <GitBranch className="w-3.5 h-3.5 text-[#6bcf8e]" />
            </span>
            <div className="min-w-0">
              <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9] leading-none">Árbol Genealógico</h1>
              <div className="mt-1 text-[10.5px] sm:text-[11px] text-[#5a6560] hidden sm:block">Cadena seed-to-sale visual · datos reales por camada</div>
            </div>
          </div>
          <div className="flex-1" />
          <span className="hidden md:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-[#3fb074]/30 bg-[#3fb074]/10 text-[#8fe0a8] text-[10.5px] uppercase tracking-widest font-medium">
          ...
          <button onClick={() => cargar(camadaSel)} disabled={cargando}
            className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-md border border-[#3fb074]/40 bg-[#3fb074]/10 hover:bg-[#3fb074]/20 transition-colors text-[11px] sm:text-[11.5px] font-medium text-[#8fe0a8] disabled:opacity-50">
      ...
      {/* Selector camadas */}
      <div className="px-3 sm:px-6 py-3 border-b border-[#1a2620] flex items-center gap-2 sm:gap-3 flex-wrap bg-[#0a0f0d] flex-shrink-0">
        <span className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium">Camada</span>
          ...
          {CAMADAS.map(c => {
            ...
            return (
              <button key={c} onClick={() => setCamadaSel(c)}
                className={['px-3 py-1.5 rounded-md text-[12px] font-medium transition-colors tabular-nums', active ? 'bg-[#3fb074] text-[#06130c]' : 'text-[#8e9892] hover:text-[#e8ece9]'].join(' ')}>
                {c}
              </button>
            )
          })}
        ...
        {!cargando && totalLotes > 0 && (
          <div className="text-[11px] text-[#5a6560]">
            <span className="text-[#8fe0a8] font-mono font-semibold tabular-nums">{totalLotes}</span> lotes · <span className="text-[#8fe0a8] font-mono font-semibold tabular-nums">{etapasUsadas.length}</span> etapas
          </div>
        )}
      </div>
      ...
      {!cargando && etapasUsadas.length > 0 && (
        <div className="px-3 sm:px-6 py-2.5 border-b border-[#1a2620] flex items-center gap-2 flex-nowrap bg-[#0a0f0d] overflow-x-auto flex-shrink-0">
          <span className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mr-1 shrink-0">Etapas</span>
          {etapasUsadas.map(e => {
            ...
            return (
              <div key={e.orden} className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md border shrink-0"
                ...
                <span className="font-mono text-[10px] text-[#5a6560] tabular-nums">{e.count}</span>
              </div>
            )
          })}
        </div>
      ...
      {cargando && (
        <div className="flex-1 flex items-center justify-center">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3, ease: EASE }}
            className="flex flex-col items-center gap-3 text-[#5a6560]">
            <Loader2 className="w-8 h-8 animate-spin text-[#6bcf8e]" />
      ...
      {!cargando && nodes.length === 0 && (
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3 text-[#5a6560] text-[12px]">
            <Sprout className="w-7 h-7 text-[#454e48]" />
