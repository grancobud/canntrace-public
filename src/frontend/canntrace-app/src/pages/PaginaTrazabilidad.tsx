...
function LotCode({ code, size = 'sm', dim = false }: { code: string; size?: 'xs' | 'sm' | 'md'; dim?: boolean }) {
  ...
  return (
    <span className={`font-mono tabular-nums tracking-tight ${cls} ${dim ? 'text-[#454e48]' : 'text-[#8e9892]'} inline-flex flex-wrap items-baseline leading-[1.4]`}>
      {parts.map((p, i) => {
        const tok = CODE_TOKENS[p]
        if (tok) return (
          <span key={i} className="relative cursor-help hover:text-[#e8ece9] hover:bg-[#3fb074]/10 rounded-[2px] transition-colors"
            ...
            <AnimatePresence>{tip === i && (
              <motion.span initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
                ...
                <span className="block px-2.5 py-1.5 rounded-md bg-[#0a0f0d] border border-[#2a3d36] shadow-[0_8px_24px_rgba(0,0,0,0.6)]">
                  <span className="block text-[9px] uppercase tracking-widest text-[#454e48] font-sans font-medium mb-0.5">{tok.label}</span>
                  <span className="block text-[11px] text-[#d3d8d4] font-sans font-medium">{tok.detail}</span>
                </span>
              </motion.span>
            )}</AnimatePresence>
          </span>
        )
        return <span key={i} className="text-[#343c37]">{p}</span>
      })}
    </span>
  )
}
...
function Pill({ children, tone = 'neutral' }: { children: React.ReactNode; tone?: 'neutral' | 'primary' | 'gold' | 'ok' | 'warn' }) {
  const tones: Record<string, string> = {
    neutral: 'bg-[#111916] text-[#8e9892] border border-[#1a2620]',
    primary: 'bg-[#3fb074]/10 text-[#8fe0a8] border border-[#3fb074]/20',
    gold: 'bg-[#C49A2C]/10 text-[#E3B94A] border border-[#C49A2C]/25',
    ok: 'bg-[#1f3a2a] text-[#6fd28a] border border-[#2a5138]',
    warn: 'bg-[#3a2f15] text-[#e3b94a] border border-[#5a4820]',
  }
...
function DurationPill({ days }: { days: number }) {
  const tone = days < 30 ? 'bg-[#1f3a2a] text-[#6fd28a] border-[#2a5138]/80'
    : days < 60 ? 'bg-[#3a2f15] text-[#e3b94a] border-[#5a4820]/80'
...
function Label({ children, tone = 'ink-40', className = '' }: { children: React.ReactNode; tone?: 'ink-40' | 'ink-50' | 'gold' | 'primary'; className?: string }) {
  const map = { 'ink-40': 'text-[#454e48]', 'ink-50': 'text-[#5a6560]', gold: 'text-[#C49A2C]', primary: 'text-[#6bcf8e]' }
...
function ProgressBar({ current, total = 10, completed = false, showLabel = true }: { current: number; total?: number; completed?: boolean; showLabel?: boolean }) {
  ...
  const accent = completed ? '#E3B94A' : '#6bcf8e'
  return (
    <div className="w-full">
      {showLabel && (
        <div className="flex items-baseline justify-between mb-1.5">
          <span className="font-mono tabular-nums text-[11px] text-[#5a6560]">
            <span className={completed ? 'text-[#E3B94A]' : 'text-[#8fe0a8]'}>{current}</span>
            <span className="text-[#454e48]">/{total}</span>
            <span className="text-[#454e48] mx-1.5">·</span>
            <span className="text-[#b2bab4] font-medium"><CountUp to={pct} />%</span>
          </span>
          <span className="text-[10px] uppercase tracking-[0.14em] font-medium text-[#454e48]">{completed ? 'Completada' : 'En progreso'}</span>
        </div>
      )}
      <div className="relative w-full h-[5px] rounded-full bg-[#0f1714] border border-[#1a2620] overflow-hidden">
        <div className="absolute inset-0 flex">
          {Array.from({ length: total - 1 }).map((_, i) => <div key={i} style={{ left: `${((i + 1) / total) * 100}%` }} className="absolute top-0 bottom-0 w-px bg-[#1a2620]" />)}
        </div>
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ ...SPRING_PROGRESS, delay: 0.15 }}
          className="relative h-full rounded-full"
          style={{
            background: completed
              ? 'linear-gradient(90deg, #3fb074 0%, #6bcf8e 35%, #E3B94A 100%)'
              : 'linear-gradient(90deg, #216b45 0%, #3fb074 45%, #6bcf8e 100%)',
...
function CircleProgress({ current, total = 10, completed = false, size = 54 }: { current: number; total?: number; completed?: boolean; size?: number }) {
  ...
  const color = completed ? '#E3B94A' : '#6bcf8e'
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={r} stroke="#1a2620" strokeWidth="4" fill="none" />
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-mono tabular-nums text-[12px] font-semibold text-[#e8ece9]">{current}<span className="text-[#454e48]">/{total}</span></span>
      </div>
    </div>
  )
}
    ...
    <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
      <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
        <div className="min-w-0">
          <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Trazabilidad Inversa</h1>
          <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
            <span className="tabular-nums">{totalChains}</span> cadenas
            <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span> Sale-to-Seed <span className="text-[#343c37] mx-1">│</span> Datos reales por etapa</span>
          </div>
        </div>
        ...
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-[#3fb074]/30 bg-[#3fb074]/10 text-[#8fe0a8] text-[10.5px] uppercase tracking-widest font-medium">GAMP5</span>
      </div>
    </div>
  )
}
...
function EstadoBar({ activas, total, onSync, onExport, syncing, exporting, onNueva }: { activas: number; total: number; onSync: () => void; onExport: () => void; syncing: boolean; exporting: boolean; onNueva: () => void }) {
  ...
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] px-3 sm:px-5 py-3 sm:py-4 flex items-center gap-2 sm:gap-4 flex-wrap">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-display font-semibold text-[14px] text-[#e8ece9]">Estado de cadenas</span>
        </div>
        <div className="mt-0.5 text-[11.5px] text-[#5a6560]">
          Última sincronización: <span className="font-mono text-[#b2bab4]">{horaSync}</span>
          <span className="text-[#343c37] mx-1.5">/</span>
          <span className="text-[#8fe0a8]">{activas} cadenas activas</span>
          <span className="text-[#343c37] mx-1.5">/</span>
          <span className="text-[#8e9892]">{total} totales</span>
        </div>
      </div>
      <div className="flex-1" />
      ...
      <button onClick={onExport} disabled={exporting}
        ...
        <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M3 2h7l3 3v9H3V2Z" stroke="#3fb074" strokeWidth="1.2" strokeLinejoin="round" /><path d="M5.5 9h5M5.5 11h5" stroke="#3fb074" strokeWidth="1.2" strokeLinecap="round" /></svg>
...
function CodeAccordion() {
  ...
  const colorMap: Record<string, string> = {
    primary: 'bg-[#3fb074]/10 border-[#3fb074]/30 text-[#8fe0a8]',
    gold: 'bg-[#C49A2C]/10 border-[#C49A2C]/30 text-[#E3B94A]',
    ink: 'bg-[#1a2620] border-[#243429] text-[#d3d8d4]',
  }
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-3 px-5 py-4 hover:bg-[#111916] transition-colors text-left">
        <motion.span animate={{ rotate: open ? 90 : 0 }} transition={{ duration: 0.2, ease: EASE }} className="text-[#5a6560]">
        ...
        <span className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-[#3fb074]/10 border border-[#3fb074]/20">
          <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M8 1.5 14 5v6l-6 3.5L2 11V5l6-3.5Z" stroke="#6bcf8e" strokeWidth="1.2" strokeLinejoin="round" /><path d="M2 5l6 3.5m0 0L14 5M8 8.5v6" stroke="#6bcf8e" strokeWidth="1.2" /></svg>
        </span>
        <span className="font-display font-semibold text-[14px] text-[#e8ece9]">Tabla Maestra de Códigos</span>
        <span className="flex-1" />
        <span className="text-[12px] text-[#5a6560] hidden md:inline">Cómo se forman los códigos de trazabilidad</span>
      </button>
      <AnimatePresence initial={false}>{open && (
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
          transition={{ duration: 0.3, ease: EASE }} className="overflow-hidden border-t border-[#1a2620]">
          <div className="px-5 py-5">
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-2 text-center">Planta individual · vegetativo y floración</div>
              ...
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
                {example.map((seg, i) => (
                  <span key={i} className="contents">
                    ...
                    {i < example.length - 1 && <span className="text-[#343c37] mx-1">.</span>}
                  </span>
                ))}
              </div>
            </div>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-6">
              {example.map((seg, i) => (
                <div key={i} className="rounded-md bg-[#111916] border border-[#1e2d27] px-3 py-3 flex flex-col items-center justify-between min-h-[78px]">
                  <div className="text-[9px] uppercase tracking-[0.14em] text-[#454e48] font-medium text-center leading-tight">{seg.l}</div>
                  <div className={`my-1 font-mono font-semibold text-[12px] text-center ${colorMap[seg.c].split(' ')[2]}`}>{seg.v}</div>
                  <div className="text-[10px] text-[#747e78] leading-tight text-center">{seg.d}</div>
                </div>
              ))}
            </div>
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-2 text-center">Post-cosecha · cuadro de secado</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
                {examplePost.map((seg, i) => (
                  <span key={i} className="contents">
                    <span className={`px-2 py-0.5 rounded border font-semibold ${colorMap[seg.c]}`}>{seg.v}</span>
                    {i < examplePost.length - 1 && <span className="text-[#343c37] mx-1">.</span>}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-2 text-center">Fraccionamiento · lote almacén + bolsas</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
                {exampleFracDep.map((seg, i) => (
                  <span key={i} className="contents">
                    <span className={`px-2 py-0.5 rounded border font-semibold ${colorMap[seg.c]}`}>{seg.v}</span>
                    {i < exampleFracDep.length - 1 && <span className="text-[#343c37] mx-1">-</span>}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-2 text-center">Depósito final · bolsa comercial</div>
            <div className="flex justify-center mb-4">
              <div className="font-mono tabular-nums text-[13px] sm:text-[18px] tracking-tight flex items-baseline flex-wrap justify-center gap-y-1">
                {exampleFis.map((seg, i) => (
                  <span key={i} className="contents">
                    <span className={`px-2 py-0.5 rounded border font-semibold ${colorMap[seg.c]}`}>{seg.v}</span>
                    {i < exampleFis.length - 1 && <span className="text-[#343c37] mx-1">.</span>}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-lg bg-[#0a0f0d] border border-[#1a2620] p-4 mb-5">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-3">Código por etapa del ciclo</div>
              <div className="flex flex-col gap-1.5">
                {patterns.map((p, i) => (
                  <div key={p.stage} className="flex items-baseline gap-3 py-0.5">
                    <span className="font-mono text-[#454e48] text-[11px] w-[18px] shrink-0 text-right">{i + 1}.</span>
                    <span className="text-[#a7b1ab] text-[12px] w-[140px] shrink-0">{p.stage}</span>
                    <span className="font-mono text-[#8fe0a8] text-[11.5px] flex-1">{p.code}</span>
                    {p.note && <span className="text-[10px] text-[#5a6560] italic">{p.note}</span>}
                  </div>
                ))}
              </div>
            </div>
            <div className="mt-5 pt-4 border-t border-[#1a2620] space-y-5">
              {[
                {
                ...
                <div key={g.titulo}>
                  <div className="text-[10px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-1">{g.titulo}</div>
                  <div className="text-[11px] text-[#747e78] mb-2.5 leading-snug">{g.desc}</div>
                  <div className="flex flex-col gap-1.5">
                    {g.items.map(([k, v]) => (
                      <div key={k} className="flex items-baseline gap-3 py-0.5">
                        <span className="font-mono text-[#8fe0a8] w-[140px] shrink-0 text-[11px]">{k}</span>
                        <span className="text-[#a7b1ab] text-[12px] leading-snug flex-1">{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
...
function FilterTabs({ filter, setFilter, system, setSystem, counts }: { filter: 'todas' | 'proceso' | 'completadas'; setFilter: (v: 'todas' | 'proceso' | 'completadas') => void; system: 'todos' | 'RDWC' | 'COCO'; setSystem: (v: 'todos' | 'RDWC' | 'COCO') => void; counts: { todas: number; proceso: number; completadas: number } }) {
  const btn = (active: boolean, onClick: () => void, children: React.ReactNode) => (
    <button onClick={onClick} className={['px-3.5 py-1.5 rounded-md text-[12px] font-medium transition-colors', active ? 'bg-[#3fb074] text-[#06130c]' : 'text-[#8e9892] hover:text-[#e8ece9]'].join(' ')}>{children}</button>
  )
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <div className="inline-flex gap-1 p-1 rounded-lg bg-[#0e1411] border border-[#1a2620]">
        {btn(filter === 'todas', () => setFilter('todas'), <>Todas <span className={filter === 'todas' ? 'opacity-70' : 'text-[#454e48]'}>({counts.todas})</span></>)}
        {btn(filter === 'proceso', () => setFilter('proceso'), <>En proceso <span className={filter === 'proceso' ? 'opacity-70' : 'text-[#454e48]'}>({counts.proceso})</span></>)}
        {btn(filter === 'completadas', () => setFilter('completadas'), <>Completadas <span className={filter === 'completadas' ? 'opacity-70' : 'text-[#454e48]'}>({counts.completadas})</span></>)}
      </div>
      <div className="inline-flex gap-1 p-1 rounded-lg bg-[#0e1411] border border-[#1a2620]">
...
function ChainCard({ chain, idx, selected, onSelect }: { chain: Chain; idx: number; selected: boolean; onSelect: (id: string) => void }) {
  return (
    <motion.button type="button" onClick={() => onSelect(chain.id)}
      ...
      className={['group relative text-left rounded-lg bg-[#111916] border p-4 transition-colors',
        selected ? 'border-[#3fb074]/60 bg-[#13201b]' : 'border-[#1e2d27] hover:bg-[#162420] hover:border-[#2a3d36]',
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline justify-between gap-2">
            <span className={`font-display font-bold tracking-tight text-[17px] ${chain.completed ? 'text-[#E3B94A]' : 'text-[#8fe0a8]'}`}>{chain.id}</span>
          </div>
          <div className="mt-1">
            {chain.completed
              ...
              : <span className="inline-flex px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#3fb074]/15 text-[#8fe0a8] border border-[#3fb074]/25">En proceso</span>}
          </div>
          <div className="mt-2 flex items-center gap-1 text-[11px] text-[#747e78]">
            ...
            <span className="text-[#8e9892]">{chain.system}</span>
          </div>
          {chain.completed && chain.yieldKg !== undefined && (
            <div className="mt-1 font-mono tabular-nums text-[11px] text-[#E3B94A]">{chain.yieldKg.toFixed(2)}kg</div>
          )}
        </div>
      </div>
    </motion.button>
...
function TimelineNode({ stage, idx, isLast, onClick }: { stage: Stage; idx: number; isLast: boolean; onClick?: (s: Stage) => void }) {
  ...
  return (
    <motion.li initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      className={`relative grid grid-cols-[36px_1fr_auto] gap-4 items-start pb-4 last:pb-0 ${clickable ? 'cursor-pointer hover:bg-[#111916]/60 -mx-3 px-3 rounded-md transition-colors' : ''}`}>
      {!isLast && <span className={`absolute left-[17px] top-[22px] bottom-0 w-px ${isDone || isActive ? 'bg-[#3fb074]/30' : 'bg-[#1a2620]'}`} />}
      <div className="relative flex items-center justify-center h-[22px] w-[36px]">
        {isActive ? (<>
          <span className="relative z-10 w-[11px] h-[11px] rounded-full bg-[#6bcf8e]" style={{ boxShadow: '0 0 10px rgba(107,207,142,0.7)' }} />
          <span className="absolute inset-0 m-auto w-[11px] h-[11px] rounded-full ring-2 ring-[#3fb074]/30 ring-offset-2 ring-offset-[#0a0f0d] animate-[ct-pulse_2.2s_ease-in-out_infinite]" />
        </>) : isDone
          ? <span className="w-[9px] h-[9px] rounded-full bg-[#6bcf8e]/80 border-2 border-[#0a0f0d]" />
          : <span className="w-[7px] h-[7px] rounded-full bg-[#1a2620] border border-[#243429]" />}
      </div>
      <div className="min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="font-mono tabular-nums text-[10.5px] text-[#454e48]">{num}</span>
          <h4 className={`font-display font-semibold text-[13.5px] ${isPending ? 'text-[#454e48]' : 'text-[#e8ece9]'}`}>{stageLabel}</h4>
          {isActive && <Pill tone="primary">en curso</Pill>}
          {clickable && <span className="text-[9px] text-[#454e48] uppercase tracking-widest ml-1">click ver detalle</span>}
        </div>
        {stage.code ? <div className="mt-1"><LotCode code={stage.code} /></div> : <div className="mt-1 text-[11px] text-[#343c37]">sin registro</div>}
        {stage.detail && <div className="mt-1 text-[11.5px] text-[#747e78]">{stage.detail}</div>}
        {stage.range && <div className="mt-1 font-mono tabular-nums text-[10.5px] text-[#454e48]">{stage.range}</div>}
      </div>
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 20 }}
          ...
          className="bg-[#0e1411] border-t sm:border border-[#1a2620] rounded-t-2xl sm:rounded-2xl shadow-2xl max-w-3xl w-full h-[92vh] sm:h-auto sm:max-h-[85vh] overflow-hidden flex flex-col"
          onClick={e => e.stopPropagation()}>
          <div className="px-4 sm:px-6 py-3.5 sm:py-5 border-b border-[#1a2620] bg-[#111916] relative">
            {/* Drag handle visible solo mobile */}
            <div className="sm:hidden absolute top-1.5 left-1/2 -translate-x-1/2 w-10 h-1 rounded-full bg-[#243429]" />
            {/* Botón cerrar en mobile (touch target grande) */}
            <button
              onClick={onClose}
              aria-label="Cerrar"
              className="sm:hidden absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center bg-[#0a0f0d] border border-[#1a2620] text-[#8e9892] active:bg-[#162420]"
            >
            ...
            <div className="flex items-baseline gap-2 sm:gap-3 flex-wrap pr-10 sm:pr-0 mt-1.5 sm:mt-0">
              <span className="font-mono tabular-nums text-[10.5px] sm:text-[11px] text-[#454e48]">STAGE {num}</span>
              <h3 className="font-display font-bold text-[17px] sm:text-[20px] text-[#e8ece9]">{stageLabel}</h3>
            ...
            <div className="mt-1.5 sm:mt-2 text-[11.5px] sm:text-[12px] text-[#8e9892]">
              Camada <span className="text-[#8fe0a8] font-semibold">{chain.id}</span> · {chain.roomLabel} · <span className="hidden xs:inline">{chain.genetic}</span><span className="xs:hidden">{chain.genetic.split(' ')[0]}</span>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 ct-page-scroll">
            ...
            {meta.tipo === 'planta_madre' && (
              <section>
                ...
                <div className="mt-3 text-[11px] text-[#5a6560] italic">
                  Nota: la planta madre <span className="font-mono text-[#b2bab4]">{meta.cod}</span> sigue activa después de clonar a esta camada. Otras camadas también derivan de ella.
                </div>
              </section>
            )}
              ...
              <>
                {meta.codigosFmt && (
                  <section>
                    ...
                    <div className="mt-2 px-3 py-2 rounded-md bg-[#111916] border border-[#1a2620] font-mono text-[11px] text-[#d3d8d4]">
                      <div className="text-[#8fe0a8]">{meta.codigosFmt.first}</div>
                      {meta.codigosFmt.last && (
                        <>
                          <div className="text-[#5a6560] my-0.5">↓ <span className="not-italic text-[#454e48]">+{Math.max(0, meta.codigosFmt.count - 2)} códigos intermedios</span></div>
                          <div className="text-[#E3B94A]">{meta.codigosFmt.last}</div>
                        </>
                      )}
                    </div>
                    <div className="mt-2 text-[11px] text-[#5a6560]">
            ...
            {meta.tipo === 'cosecha' && (
              <section>
                ...
                <div className="mt-2 text-[11px] text-[#5a6560]">Cosecha no genera traza nueva: las plantas mantienen su código de floración.</div>
              </section>
            )}
            ...
            {meta.tipo === 'secado' && (
              <section>
                ...
                {meta.cuadros?.length > 0 && (
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-1.5 text-[11px] font-mono text-[#b2bab4]">
                    {meta.cuadros.slice(0, 12).map((c: any) => <div key={c.id} className="px-2 py-1 bg-[#111916] rounded border border-[#1a2620]">{c.codigo_lote}</div>)}
                  </div>
                )}
              </section>
            )}
            ...
            {meta.tipo === 'trimming' && (
              <section>
                ...
                {meta.lotes?.length > 0 && (
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-1.5 text-[11px] font-mono text-[#b2bab4]">
                    {meta.lotes.slice(0, 12).map((l: any) => <div key={l.id} className="px-2 py-1 bg-[#111916] rounded border border-[#1a2620] flex justify-between"><span>{l.codigo_lote}</span><span className="text-[#E3B94A]">{Number(l.cantidad || 0).toLocaleString('es-AR')} g</span></div>)}
                  </div>
                )}
              </section>
            )}
            ...
            {meta.tipo === 'cuarentena' && (
              <section>
                ...
                {meta.calidad?.length > 0 && (
                  <div className="mt-3 space-y-1.5 text-[11px] text-[#b2bab4]">
                    {meta.calidad.slice(0, 8).map((c: any) => (
                      <div key={c.id} className="px-2 py-1.5 bg-[#111916] rounded border border-[#1a2620]">
                        <div className="flex items-baseline gap-2">
                          <span className="font-mono text-[#8fe0a8]">{c.numero_certificado || c.tipo}</span>
                          <span className="font-mono text-[10px] text-[#5a6560]">{c.fecha}</span>
                          {c.resultado && <Pill tone={c.resultado === 'aprobado' ? 'ok' : 'warn'}>{c.resultado}</Pill>}
                        </div>
                        {c.datos_extra?.lote_codigo && <div className="mt-0.5 font-mono text-[10px] text-[#5a6560]">lote: {c.datos_extra.lote_codigo}</div>}
                      </div>
                    ))}
                  </div>
                )}
            ...
            {meta.tipo === 'deposito' && (
              <section>
                ...
                {meta.trazas?.length > 0 && (
                  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-1.5 text-[11px] font-mono text-[#b2bab4]">
                    {meta.trazas.slice(0, 12).map((t: any) => <div key={t.id} className="px-2 py-1 bg-[#111916] rounded border border-[#1a2620]">{t.cod_comercial}</div>)}
                    {meta.trazas.length > 12 && <div className="px-2 py-1 text-[#5a6560] italic">...y {meta.trazas.length - 12} más</div>}
                  </div>
                )}
              </section>
            )}
          ...
          <div className="px-6 py-3 border-t border-[#1a2620] bg-[#111916] flex items-center justify-between">
            <div className="text-[10.5px] text-[#5a6560] font-mono">Stage {num} · {stageLabel}</div>
            <button onClick={onClose} className="px-3 py-1.5 rounded-md border border-[#1a2620] text-[12px] text-[#d3d8d4] hover:bg-[#162420]">Cerrar</button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
...
function KV({ label, value, mono }: { label: string; value: any; mono?: boolean }) {
  return (
    <div>
      <div className="text-[9.5px] uppercase tracking-[0.14em] text-[#5a6560]">{label}</div>
      <div className={`mt-0.5 text-[12.5px] text-[#e8ece9] ${mono ? 'font-mono tabular-nums' : ''}`}>{value ?? '—'}</div>
    </div>
  )
}
...
function AmbientalesSummary({ ambs }: { ambs: any[] }) {
  ...
  return (
    <div className="mt-2 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
      {stats.map(s => (
        <div key={s.key} className="px-3 py-2 rounded-md bg-[#111916] border border-[#1a2620]">
          <div className="text-[9.5px] uppercase tracking-widest text-[#5a6560]">{s.label}</div>
          <div className="mt-0.5 font-mono tabular-nums text-[11.5px]">
            <span className="text-[#E3B94A] font-semibold">{s.avg}</span><span className="text-[#5a6560]"> {s.unit}</span>
          </div>
          <div className="text-[9.5px] font-mono tabular-nums text-[#5a6560]">{s.min} – {s.max}</div>
        </div>
      ))}
    </div>
  )
...
function DetailPanel({ chain, onStageClick }: { chain: Chain | undefined; onStageClick?: (s: Stage) => void }) {
  ...
  return (
    <motion.div key={chain.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.3, ease: EASE }} className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
      <div className="px-5 py-4 border-b border-[#1a2620]">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h2 className="font-display font-bold tracking-tight text-[22px] text-[#e8ece9] leading-none">
              Camada <span className="text-[#8fe0a8]">{chain.id}</span><span className="text-[#454e48] font-medium ml-1.5">· {chain.system}</span>
            </h2>
            <div className="mt-1.5 text-[12px] text-[#747e78]">
              <span className="text-[#b2bab4]">{chain.genetic}</span>
              {chain.start && (
                <> · <span className="font-mono tabular-nums text-[#b2bab4]">{chain.start}</span>
                  {chain.end && chain.end !== chain.start
                    ? <> → <span className="font-mono tabular-nums text-[#b2bab4]">{chain.end}</span></>
                    : (chain.completed ? null : <span className="text-[#5a6560]"> → vigente</span>)}
                </>
              )}
          ...
          <div className="text-right shrink-0">
            <Label>CUMCS</Label>
            <div className="mt-0.5 font-mono tabular-nums text-[12px] text-[#b2bab4]">{chain.code}</div>
          </div>
        </div>
...
function ComparisonPanel({ chains }: { chains: Chain[] }) {
  ...
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[#1a2620] flex items-baseline justify-between">
        <div>
          <Label tone="primary">Comparación</Label>
          <div className="mt-0.5 font-display font-semibold text-[13.5px] text-[#e8ece9]">Rendimiento histórico</div>
        </div>
        <span className="text-[10.5px] text-[#5a6560] font-mono">{completed.length} camadas</span>
      </div>
          ...
          return (
            <div key={c.id}>
              <div className="flex items-baseline justify-between mb-1">
                <span className="font-display font-semibold text-[12.5px] text-[#e8ece9]">Camada {c.id}</span>
                <span className="font-mono tabular-nums text-[11.5px] text-[#E3B94A] font-medium">{(c.yieldKg || 0).toFixed(1)}<span className="text-[#5a6560] text-[10px] ml-0.5">kg</span></span>
              </div>
              <div className="relative h-[6px] rounded-full bg-[#111916] overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ ...SPRING_PROGRESS, delay: 0.2 + i * 0.05 }}
                  className="h-full rounded-full" style={{ background: 'linear-gradient(90deg, #3fb074, #E3B94A)' }} />
              </div>
              <div className="mt-1 flex items-center justify-between text-[10.5px]">
                <span className="text-[#5a6560] font-mono">{c.system} · {c.start.slice(0, 7)}</span>
                {delta !== null && (
                  <span className={`font-mono tabular-nums ${delta >= 0 ? 'text-[#6fd28a]' : 'text-[#e08282]'}`}>
...
function QuickStats({ chains }: { chains: Chain[] }) {
  ...
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[#1a2620]">
        <Label tone="primary">KPIs · CUMCS</Label>
        <div className="mt-0.5 font-display font-semibold text-[13.5px] text-[#e8ece9]">Vista general</div>
      </div>
      <div className="divide-y divide-[#1a2620]">
        {items.map((k, i) => (
          <div key={i} className="px-5 py-3 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-[10px] uppercase tracking-[0.14em] text-[#454e48] font-medium">{k.label}</div>
              <div className="mt-0.5 text-[11px] text-[#747e78]">{k.hint}</div>
            </div>
            <div className="font-display font-bold tracking-tight text-[20px] text-[#e8ece9] leading-none text-right shrink-0">
              {k.valueText !== undefined ? <span>{k.valueText}</span> : <><CountUp to={k.value || 0} decimals={k.decimals || 0} />{k.suffix && <span className="text-[#5a6560] font-medium text-[13px]">{k.suffix}</span>}</>}
            </div>
          </div>
        ))}
      </div>
...
export default function PaginaTrazabilidad() {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="flex-1 overflow-y-auto ct-page-scroll bg-[#0a0f0d] text-[#d3d8d4] font-sans">
        ...
        <div className="px-3 sm:px-6 py-3 sm:py-5 space-y-3 sm:space-y-4 pb-20">
          ...
          {loading ? (
            ...
            <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] py-4">
              <EmptyState
                icon={Sprout}
                ...
                secondaryAction={
                  <Link
                    to="/importador"
                    className="inline-flex items-center gap-1.5 px-4 py-2 rounded-md border border-[#1e2d27] bg-[#111916] hover:bg-[#162420] text-[#d3d8d4] text-[12.5px] font-medium transition-colors">
            ...
            <div className="grid grid-cols-12 gap-4">
              <div className="col-span-12 lg:col-span-8 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                  ...
                  {filtered.length === 0 && (
                    <div className="col-span-full rounded-xl border border-dashed border-[#1e2d27] bg-[#111916]/50 px-6 py-10 text-center">
                      <div className="mx-auto w-9 h-9 rounded-full bg-[#162420] border border-[#1e2d27] flex items-center justify-center mb-2">
                        <svg width="16" height="16" viewBox="0 0 18 18" fill="none"><path d="M9 3v12M3 9h12" stroke="currentColor" strokeWidth="1.2" className="text-[#5a6560]" /></svg>
                      </div>
                      <div className="font-display font-semibold text-[#d3d8d4] text-[13px]">Sin cadenas</div>
                      <div className="mt-0.5 text-[11px] text-[#5a6560]">Ajustá los filtros.</div>
                    </div>
                  )}
                </div>
          ...
          <footer className="pt-6 mt-2 border-t border-[#1a2620] text-[11px] text-[#5a6560] flex items-center justify-between flex-wrap gap-2">
  ...
  return (
    <Drawer.Root open={open} onOpenChange={onOpenChange} direction="right">
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed z-50 bg-[#0e1411] border-l border-[#1a2620] flex flex-col right-0 top-0 bottom-0 w-full sm:max-w-xl outline-none">
          ...
          <div className="flex items-center justify-between p-5 border-b border-[#1a2620] sticky top-0 bg-[#0e1411] z-10">
            <div>
              <h3 className="font-display font-bold text-[16px] text-[#e8ece9]">Nueva cadena</h3>
              <p className="text-[11.5px] text-[#5a6560] mt-0.5">Crear lote raíz (planta madre) e iniciar la trazabilidad.</p>
            </div>
            <button onClick={() => onOpenChange(false)} className="p-2 rounded-md hover:bg-[#162420] text-[#8e9892]">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-5 space-y-4">
            {/* Código de lote */}
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6bcf8e] font-semibold mb-1.5">Código del lote raíz</label>
              <input
                {...register('codigo_lote')}
                placeholder="Ej: 26.PM10"
                autoComplete="off"
                className="w-full px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] rounded-md text-[13px] font-mono tabular-nums text-[#e8ece9] placeholder:text-[#454e48] focus:outline-none focus:border-[#3fb074]/60"
              />
              {sugerenciasLote.length > 0 && (
                <div className="mt-1.5 rounded-md border border-[#1a2620] bg-[#0a0f0d] divide-y divide-[#1a2620]">
                  {sugerenciasLote.map(s => (
                    <button
                      type="button"
                      key={s.codigo_lote}
                      onClick={() => setValue('codigo_lote', s.codigo_lote, { shouldValidate: true })}
                      className="w-full text-left px-3 py-1.5 text-[12px] font-mono text-[#8fe0a8] hover:bg-[#111916]"
                    >
                      {s.codigo_lote}
                    </button>
                  ))}
            ...
            {/* Variedad */}
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6bcf8e] font-semibold mb-1.5">Variedad</label>
              <Controller
                control={control}
                name="variedad_id"
                render={({ field }) => (
                  <select
                    ...
                    className="w-full px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] rounded-md text-[13px] text-[#e8ece9] focus:outline-none focus:border-[#3fb074]/60 disabled:opacity-50"
                  >
            ...
            {/* Instalacion */}
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6bcf8e] font-semibold mb-1.5">Instalación origen</label>
              <Controller
                control={control}
                name="instalacion_id"
                render={({ field }) => (
            ...
            {/* Fecha inicio */}
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6bcf8e] font-semibold mb-1.5">Fecha de inicio</label>
              <input
                type="date"
                {...register('fecha_inicio')}
                className="w-full px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] rounded-md text-[13px] font-mono text-[#e8ece9] focus:outline-none focus:border-[#3fb074]/60"
              />
            ...
            {/* Notas */}
            <div>
              <label className="block text-[11px] uppercase tracking-widest text-[#6bcf8e] font-semibold mb-1.5">Notas (opcional)</label>
              <textarea
                {...register('notas')}
                rows={3}
                placeholder="Origen genético, observaciones iniciales…"
                className="w-full px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] rounded-md text-[13px] text-[#d3d8d4] placeholder:text-[#454e48] focus:outline-none focus:border-[#3fb074]/60 resize-none"
              />
              {errors.notas && <p className="mt-1 text-[11px] text-[#e08282]">{errors.notas.message}</p>}
            </div>
          </form>

          <div className="px-5 py-4 border-t border-[#1a2620] flex items-center gap-2 bg-[#0e1411]">
            <button
              type="button"
              onClick={() => onOpenChange(false)}
              disabled={isSubmitting}
              className="px-3 py-2 rounded-md border border-[#1a2620] bg-[#111916] hover:bg-[#162420] text-[12.5px] font-medium text-[#d3d8d4] disabled:opacity-50"
            >Cancelar</button>
            <div className="flex-1" />
            <button
              type="submit"
