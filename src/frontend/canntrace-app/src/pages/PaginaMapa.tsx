...
const TONOS: Record<Area['tono'], { fill: string; fillSel: string; stroke: string; strokeSel: string; text: string }> = {
  'primary':       { fill: 'rgba(63,176,116,0.10)',  fillSel: 'rgba(63,176,116,0.22)',  stroke: '#2a5138', strokeSel: '#6bcf8e', text: '#8fe0a8' },
  'primary-soft':  { fill: 'rgba(63,176,116,0.05)',  fillSel: 'rgba(63,176,116,0.16)',  stroke: '#243429', strokeSel: '#6bcf8e', text: '#8fe0a8' },
  'gold':          { fill: 'rgba(196,154,44,0.10)',  fillSel: 'rgba(227,185,74,0.22)',  stroke: '#5a4820', strokeSel: '#E3B94A', text: '#E3B94A' },
  'gold-soft':     { fill: 'rgba(196,154,44,0.05)',  fillSel: 'rgba(227,185,74,0.16)',  stroke: '#3a2f15', strokeSel: '#E3B94A', text: '#C49A2C' },
  'muted':         { fill: 'rgba(180,200,190,0.04)', fillSel: 'rgba(180,200,190,0.10)', stroke: '#1a2620', strokeSel: '#8e9892', text: '#8e9892' },
}
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Mapa de Instalaciones</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]"><span className="hidden sm:inline">Plano fisico FIS S.A.S. · </span>Clickea un area para ver lotes</div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-3 sm:py-5 pb-20 space-y-3 sm:space-y-4">
        {cargando && <div className="text-center py-12"><Loader2 className="w-10 h-10 text-[#6bcf8e] animate-spin mx-auto" /></div>}
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={i} className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
                  <div className="text-[10px] uppercase tracking-[0.14em] text-[#454e48] font-medium">{k.label}</div>
                  <div className="mt-1 font-display font-bold tracking-tight text-[24px] text-[#e8ece9] leading-none tabular-nums">{k.value}</div>
                  <div className="mt-1 text-[11px] text-[#747e78]">{k.hint}</div>
                </div>
              ))}
            </div>

            {/* Plano */}
            <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5 overflow-x-auto">
              <div className="flex items-baseline justify-between mb-4">
                <div>
                  <div className="text-[10px] uppercase tracking-[0.14em] text-[#6bcf8e] font-medium">Plano arquitectonico</div>
                  <div className="mt-0.5 font-display font-semibold text-[14px] text-[#e8ece9]">Indoor FIS · 9 areas operativas</div>
                </div>
                <div className="flex items-center gap-2 text-[10.5px] text-[#5a6560]">
                  <MapPin className="w-3.5 h-3.5 text-[#6bcf8e]" />
                  <span>Click en un area para detalle</span>
                </div>
              </div>

              <svg viewBox="0 0 800 530" className="w-full max-w-full" style={{ background: '#0a0f0d', borderRadius: 8 }}>
                {/* Marco exterior */}
                <rect x="20" y="20" width="760" height="490" fill="none" stroke="#1a2620" strokeWidth="1" strokeDasharray="6,4" rx="4" />
                {/* Etiqueta marco */}
                <text x="40" y="38" fill="#454e48" fontSize="10" letterSpacing="2" fontFamily="system-ui">FIS S.A.S. · ROSARIO · INDOOR PRINCIPAL</text>

                {/* Lineas guia tenues entre filas */}
                <line x1="40" y1="170" x2="760" y2="170" stroke="#1a2620" strokeWidth="1" strokeDasharray="2,4" />
                <line x1="40" y1="360" x2="760" y2="360" stroke="#1a2620" strokeWidth="1" strokeDasharray="2,4" />

                {/* Etiquetas de fila */}
                <text x="30" y="115" fill="#454e48" fontSize="9" letterSpacing="2" fontFamily="system-ui" transform="rotate(-90 30 115)">PROPAGACION</text>
                <text x="30" y="270" fill="#454e48" fontSize="9" letterSpacing="2" fontFamily="system-ui" transform="rotate(-90 30 270)">FLORACION</text>
                <text x="30" y="425" fill="#454e48" fontSize="9" letterSpacing="2" fontFamily="system-ui" transform="rotate(-90 30 425)">POSTCOSECHA</text>
                ...
                {AREAS.map(a => {
                  ...
                  return (
                    <g key={a.id} onClick={() => setAreaSel(isSel ? null : a.id)} style={{ cursor: 'pointer' }}>
                      ...
                      {/* nombre central */}
                      <text x={a.x + a.w / 2} y={a.y + a.h / 2 - 4} textAnchor="middle" fill="#e8ece9" fontSize="12" fontFamily="system-ui" fontWeight="500">{a.nombre}</text>
                      {/* metrica especifica por area */}
                      {m && (
                        <text x={a.x + a.w / 2} y={a.y + a.h / 2 + 14} textAnchor="middle" fill={m.resaltado ? t.text : '#454e48'} fontSize="11" fontFamily="JetBrains Mono, ui-monospace, monospace" fontWeight="600">
                          {m.texto}
                        </text>
                      )}
                    </g>
                ...
                <g transform="translate(40, 490)">
                  <rect x="0" y="0" width="10" height="10" fill="rgba(63,176,116,0.16)" stroke="#2a5138" strokeWidth="1" rx="2" />
                  <text x="16" y="9" fill="#8e9892" fontSize="10" fontFamily="system-ui">Propagacion</text>
                  <rect x="120" y="0" width="10" height="10" fill="rgba(196,154,44,0.16)" stroke="#5a4820" strokeWidth="1" rx="2" />
                  <text x="136" y="9" fill="#8e9892" fontSize="10" fontFamily="system-ui">Floracion / cuarentena</text>
                  <rect x="290" y="0" width="10" height="10" fill="rgba(63,176,116,0.10)" stroke="#243429" strokeWidth="1" rx="2" />
                  <text x="306" y="9" fill="#8e9892" fontSize="10" fontFamily="system-ui">Postcosecha</text>
                </g>
              </svg>
            </div>

            {/* Instalaciones BD compactas */}
            <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5">
              <div className="flex items-center gap-2 mb-3">
                <Building2 className="w-4 h-4 text-[#6bcf8e]" />
                <span className="font-display font-semibold text-[14px] text-[#e8ece9]">Instalaciones registradas</span>
                <span className="text-[10.5px] text-[#5a6560] font-mono ml-auto">{datos.instalaciones.length} totales</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {datos.instalaciones.map(i => (
                  <div key={i.id} className="bg-[#111916] border border-[#1a2620] rounded-lg p-3 text-[12px] hover:border-[#3fb074]/30 transition-colors">
                    <div className="font-medium text-[#e8ece9] truncate">{i.nombre}</div>
                    <div className="text-[10px] text-[#5a6560] mt-0.5 font-mono">{i.codigo} · {i.tipo}</div>
                    <div className="flex gap-3 mt-2 text-[10.5px]">
                      <span className="text-[#5a6560]">Lotes <span className="text-[#8fe0a8] font-mono font-semibold">{i.lotes_activos}</span></span>
                      <span className="text-[#5a6560]">Unid <span className="text-[#E3B94A] font-mono font-semibold">{i.total_unidades}</span></span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
                ...
                className="bg-[#0e1411] border-l border-[#1a2620] w-full max-w-md h-full overflow-y-auto"
                onClick={e => e.stopPropagation()}>
                <div className="px-5 py-4 border-b border-[#1a2620] bg-[#111916] flex items-center justify-between">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-display font-bold text-[15px] text-[#e8ece9] truncate">{area.nombre}</h3>
                    </div>
                    <p className="text-[10.5px] text-[#5a6560] mt-1 font-mono">{lotes} lote{lotes === 1 ? '' : 's'} · {area.cumcs?.length || 0} CUMCS asociados</p>
                  </div>
                  <button onClick={() => setAreaSel(null)} className="p-1.5 rounded-md hover:bg-[#162420] text-[#8e9892] shrink-0">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="p-5 space-y-4">
                  {area.cumcs && area.cumcs.length > 0 && (
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.14em] text-[#6bcf8e] font-medium mb-2">CUMCS asociados</div>
                      <div className="flex flex-wrap gap-1.5">
                        {area.cumcs.map(c => (
                          <span key={c} className="inline-flex px-2 py-1 rounded font-mono text-[11px] bg-[#3fb074]/10 text-[#8fe0a8] border border-[#3fb074]/20">{c}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    <div className="text-[10px] uppercase tracking-[0.14em] text-[#6bcf8e] font-medium mb-2">Lotes alojados</div>
                    {detalle.length === 0 ? (
                      <div className="text-center py-8 text-[#5a6560] text-[11px]">Sin lotes activos en esta area.</div>
                    ) : (
                      <table className="w-full text-[11px]">
                        <thead>
                          <tr className="text-[#5a6560] text-left border-b border-[#1a2620]">
                        ...
                        <tbody>
                          {detalle.map((l, i) => (
                            <tr key={i} className="border-b border-[#1a2620] last:border-0">
                              <td className="py-1.5 font-mono text-[#d3d8d4]">{l.camada || '—'}</td>
                              <td className="py-1.5 text-[#8e9892] capitalize">{l.tipo_producto.replace(/_/g, ' ')}</td>
                              <td className="py-1.5 text-[#8e9892] font-mono">{l.sistema}</td>
                              <td className="py-1.5 text-right font-mono text-[#8fe0a8] font-semibold">{l.total}</td>
                              <td className="py-1.5 text-right font-mono text-[#E3B94A]">{l.unidades}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
