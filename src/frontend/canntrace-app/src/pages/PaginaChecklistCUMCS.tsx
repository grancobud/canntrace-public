    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <CheckSquare className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Checklist CUMCS IMC-GAP</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">{totalCompletados}/{totalReqs} con datos ({porcentaje}%) · <span className="hidden sm:inline">cruzado con Excel CM-RE-1010</span></div>
          </div>
          ...
          <button onClick={() => setMostrarCodigos(v => !v)} title="Mostrar codigos CUMCS internos"
            className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-lg text-[11px] font-medium transition-colors ${
              mostrarCodigos
                ...
                : 'bg-[#0e1411] border-[#1a2620] hover:border-[#2a5138] text-[#b2bab4]'
            }`}>
          ...
          <button onClick={() => cargar(true)} disabled={sincronizando}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] hover:border-[#2a5138] text-[#b2bab4] rounded-lg text-[11px] font-medium disabled:opacity-50 transition-colors">
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        {/* Barra de progreso */}
        <div className="bg-[#0e1411] rounded-xl border border-[#1a2620] p-4 sm:p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-semibold text-[#e8ece9]">Progreso CUMCS — Excel + Supabase</h3>
              {ultimaSinc && (
                <p className="text-[10px] text-[#5a6560] mt-0.5">Ultima sincronizacion: {ultimaSinc.toLocaleTimeString()}</p>
              )}
            </div>
            <span className="text-2xl font-bold text-[#3fb074] ml-3 tabular-nums">{porcentaje}%</span>
          </div>
          <div className="w-full bg-[#1a2620] rounded-full h-2.5">
            <div className="bg-[#3fb074] h-2.5 rounded-full transition-all" style={{ width: `${porcentaje}%` }} />
          </div>
          <div className="flex flex-wrap gap-4 mt-3 text-[11px] text-[#5a6560]">
            <span>{totalReqs} registros totales</span>
            <span className="text-[#6bcf8e] font-medium">{totalCompletados} con datos</span>
            <span className="text-[#E3B94A]">{totalReqs - totalCompletados} sin datos</span>
          </div>
        </div>

        {/* Camadas */}
        <div className="bg-[#0e1411] rounded-xl border border-[#1a2620] p-4">
          <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-3">Camadas rastreadas</p>
          <div className="flex flex-wrap gap-2">
            {CAMADAS_EN_PROCESO.map(c => (
              <div key={c} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#2a5138] bg-[#3fb074]/8 text-xs">
                {c === 'C16' ? <Droplets className="w-3 h-3 text-[#3fb074]" /> : <Leaf className="w-3 h-3 text-[#E3B94A]" />}
                <span className="font-bold text-[#e8ece9]">{c}</span>
                <span className="text-[#6bcf8e] font-semibold">En proceso</span>
              </div>
            ))}
            {CAMADAS_COMPLETADAS.map(c => {
              const esCoco = ['C9', 'C12'].includes(c)
              return (
                <div key={c} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#243429] bg-[#111916] text-xs">
                  {esCoco ? <Leaf className="w-3 h-3 text-[#E3B94A]" /> : <Droplets className="w-3 h-3 text-[#3fb074]" />}
                  <span className="font-bold text-[#d3d8d4]">{c}</span>
                  <span className="text-[#8e9892]">Completada</span>
                </div>
              )
            })}
          </div>
        ...
        <div className="flex gap-2 sm:gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
              ...
              className="w-full pl-9 pr-4 py-2 bg-[#0e1411] border border-[#1a2620] rounded-lg text-[13px] text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138] transition-colors" />
          </div>
          <div className="flex items-center gap-0.5 bg-[#0e1411] border border-[#1a2620] rounded-lg p-1">
            {(['todos', 'completados', 'pendientes'] as const).map(f => (
              <button key={f} onClick={() => setFiltro(f)}
                className={`px-2.5 py-1.5 rounded-md text-[11px] font-medium transition-colors ${filtro === f ? 'bg-[#3fb074] text-[#0a0f0d]' : 'text-[#8e9892] hover:text-[#d3d8d4]'}`}>
        ...
        {cargando ? (
          <div className="text-center py-12"><div className="w-8 h-8 border-2 border-[#1a2620] border-t-[#3fb074] rounded-full animate-spin mx-auto" /></div>
        ) : (
          <div className="space-y-3">
            {AREAS.map(area => {
              ...
              return (
                <div key={area.id} className="rounded-xl border border-[#1a2620] bg-[#0e1411] overflow-hidden">
                  <button onClick={() => setAreaAbierta(isAreaOpen && !busqueda ? null : area.id)}
                    className="w-full flex items-center gap-3 p-4 hover:bg-[#111916] transition-colors">
                    {isAreaOpen ? <ChevronDown className="w-4 h-4 text-[#5a6560]" /> : <ChevronRight className="w-4 h-4 text-[#5a6560]" />}
                    {AreaIcon && <AreaIcon className="w-4 h-4 text-[#3fb074]" strokeWidth={1.75} />}
                    <div className="flex-1 text-left min-w-0">
                      <p className="text-sm font-semibold text-[#e8ece9]">{area.nombre}</p>
                      <p className="text-[10px] text-[#5a6560] hidden sm:block">{area.descripcion}</p>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3">
                      <span className="text-[11px] text-[#8e9892] tabular-nums">{compArea}/{totalArea}</span>
                      <div className="w-16 sm:w-24 bg-[#1a2620] rounded-full h-1.5">
                        <div className={`h-1.5 rounded-full transition-all ${pctArea === 100 ? 'bg-[#3fb074]' : pctArea > 0 ? 'bg-[#E3B94A]' : 'bg-[#243429]'}`} style={{ width: `${pctArea}%` }} />
                      </div>
                      <span className="text-sm font-bold text-[#e8ece9] w-9 text-right tabular-nums">{pctArea}%</span>
                    </div>
                  </button>

                  {isAreaOpen && (
                    <div className="border-t border-[#1a2620] bg-[#0a0f0d]/40 p-2 sm:p-3 space-y-2">
              ...
              return (
                <div key={grupo.grupo} className="bg-[#0e1411] rounded-xl border border-[#243429] overflow-hidden">
                  <button onClick={() => setGrupoAbierto(isOpen && !busqueda ? null : grupo.grupo)}
                    className="w-full flex items-center gap-3 p-3 sm:p-4 hover:bg-[#111916] transition-colors">
                    {isOpen ? <ChevronDown className="w-4 h-4 text-[#5a6560]" /> : <ChevronRight className="w-4 h-4 text-[#5a6560]" />}
                    <div className="flex-1 text-left min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-mono font-bold text-[#3fb074] bg-[#3fb074]/10 px-2 py-0.5 rounded">{grupo.grupo}</span>
                        <h3 className="text-sm font-semibold text-[#e8ece9] truncate">{grupo.grupo_nombre}</h3>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[11px] text-[#8e9892] tabular-nums">{grupo.completados}/{grupo.total}</span>
                      <div className="w-14 sm:w-20 bg-[#1a2620] rounded-full h-1.5">
                        <div className={`h-1.5 rounded-full transition-all ${progreso === 100 ? 'bg-[#3fb074]' : progreso > 0 ? 'bg-[#E3B94A]' : 'bg-[#243429]'}`}
                  ...
                  {isOpen && porSub.map(sub => {
                    ...
                    return (
                      <div key={sub.nombre} className="border-t border-[#1a2620]">
                        <button onClick={() => setSubAbierta(subOpen && !busqueda ? null : subKey)}
                          className="w-full flex items-center gap-2 px-4 py-2.5 pl-8 hover:bg-[#111916] bg-[#0a0f0d]/30 border-l-2 border-[#3fb074]/30 transition-colors">
                          {subOpen ? <ChevronDown className="w-3 h-3 text-[#3fb074]" /> : <ChevronRight className="w-3 h-3 text-[#3fb074]" />}
                          <span className="text-[10px] font-bold text-[#8e9892] uppercase tracking-[0.14em] flex-1 text-left">{sub.nombre}</span>
                          <span className="text-[10px] font-semibold text-[#5a6560] bg-[#111916] border border-[#243429] px-2 py-0.5 rounded-full">{subComp}/{subTotal}</span>
                        </button>
                        {subOpen && (
                        ...
                        return (
                          <div key={reg.codigo}>
                            <div className="flex items-center gap-3 px-4 py-3 pl-10 hover:bg-[#111916] cursor-pointer transition-colors"
                              onClick={() => setRegistroExpandido(isExpanded ? null : reg.codigo)}>
                              {tieneData ? (
                                <CheckSquare className="w-4 h-4 text-[#3fb074] flex-shrink-0" />
                              ) : (
                                <Square className="w-4 h-4 text-[#243429] flex-shrink-0" />
                              )}
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                  {mostrarCodigos && <span className="text-[11px] font-mono text-[#5a6560] opacity-60">{reg.codigo}</span>}
                                  {reg.obligatorio && <span className="text-[9px] font-bold text-[#ff8a7a] border border-[#7a2820]/50 px-1 rounded">OBL</span>}
                                  {tieneExcel && <span className="text-[9px] font-bold text-[#6bcf8e] border border-[#2a5138]/50 px-1 rounded">CON DATOS</span>}
                                  {(conteosReales[reg.codigo] || 0) > 0 && (
                                    <span className="text-[9px] font-bold text-[#3fb074] bg-[#3fb074]/10 px-1.5 rounded">
                                      DB: {conteosReales[reg.codigo]}
                                    </span>
                                  )}
                                </div>
                                <p className={`text-sm ${tieneData ? 'text-[#e8ece9]' : 'text-[#5a6560]'}`}>{reg.nombre}</p>
                              </div>
                              {isExpanded ? <ChevronDown className="w-3.5 h-3.5 text-[#5a6560]" /> : <ChevronRight className="w-3.5 h-3.5 text-[#5a6560]" />}
                            </div>

                            {isExpanded && (
                              <div className="px-4 py-3 pl-16 bg-[#0a0f0d]/50 border-t border-[#1a2620]">
                                {reg.procedimiento_referencia && (
                                  <p className="text-[11px] text-[#5a6560] mb-2">Procedimiento: {reg.procedimiento_referencia}</p>
                                )}
                                <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-2">Estado por camada</p>
                                  ...
                                  {todasCamadas.map(cam => {
                                    ...
                                    return (
                                      <div key={cam}
                                        ...
                                        estadoFinal === 'registrado' ? 'bg-[#3fb074]/8 border border-[#2a5138] cursor-pointer hover:bg-[#3fb074]/12 hover:border-[#3fb074]/50' :
                                        estadoFinal === 'pendiente' ? 'bg-[#E3B94A]/8 border border-[#5a4820]' :
                                        'bg-[#111916] border border-[#1a2620] opacity-40'
                                      }`}>
                                        {estadoFinal === 'registrado' ? (
                                          <CheckSquare className="w-3.5 h-3.5 text-[#3fb074]" />
                                        ) : estadoFinal === 'pendiente' ? (
                                          <Square className="w-3.5 h-3.5 text-[#E3B94A]" />
                                        ) : (
                                          <span className="text-[10px] text-[#5a6560]">—</span>
                                        )}
                                        <span className="text-xs font-bold text-[#e8ece9]">{cam}</span>
                                        {esCoco ? <Leaf className="w-3 h-3 text-[#E3B94A]" /> : <Droplets className="w-3 h-3 text-[#3fb074]" />}
                                        <span className="text-[11px] text-[#8e9892]">{esCoco ? 'COCO' : 'RDWC'}</span>
                                        <span className={`text-[10px] ml-auto font-medium flex items-center gap-1 ${
                                          estadoFinal === 'registrado' ? 'text-[#6bcf8e]' :
                                          estadoFinal === 'pendiente' ? 'text-[#E3B94A]' : 'text-[#5a6560]'
                                        }`}>
        ...
        <div className="p-3 bg-[#0e1411] border border-[#1a2620] rounded-xl text-[11px] text-[#5a6560] text-center">
      ...
      {verRegistro && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" onClick={() => setVerRegistro(null)}>
          <div className="bg-[#0e1411] border border-[#1a2620] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-[#1a2620] flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[10px] font-mono text-[#3fb074]">Camada {verRegistro.camada}{mostrarCodigos ? ` · ${verRegistro.reg.codigo}` : ''}</p>
                <h3 className="text-base font-display font-bold text-[#e8ece9] mt-0.5">{verRegistro.reg.nombre}</h3>
                <p className="text-[11px] text-[#5a6560]">
                  {verRegistro.reg.grupo_nombre} · Tabla: {tablaDelGrupo(verRegistro.reg.grupo)} · {registrosCargados.length} registro(s)
                </p>
              </div>
              <button onClick={() => setVerRegistro(null)} className="p-1.5 rounded-lg hover:bg-[#162420] text-[#5a6560] hover:text-[#d3d8d4] flex-shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              {cargandoRegistros ? (
                <div className="text-center py-8">
                  <div className="w-8 h-8 border-2 border-[#1a2620] border-t-[#3fb074] rounded-full animate-spin mx-auto" />
                  <p className="mt-3 text-sm text-[#5a6560]">Buscando registros para {verRegistro.camada}...</p>
                </div>
                ...
                registrosCargados.map((r, idx) => {
                  ...
                  return (
                    <div key={r.id || idx} className="border border-[#243429] bg-[#111916] rounded-xl overflow-hidden">
                      <div className="px-4 py-2 border-b border-[#1a2620] flex items-center justify-between">
                        <span className="text-[11px] font-mono text-[#5a6560]">#{idx + 1}</span>
                        <span className="text-[10px] text-[#5a6560]">id: {String(r.id).slice(0, 8)}...</span>
                      </div>
                      <div className="p-4 grid grid-cols-2 gap-x-4 gap-y-2">
                        {campos.map(([k, v]) => (
                          <div key={k}>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium">{k}</p>
                            <p className="text-sm text-[#d3d8d4] break-words">{typeof v === 'object' ? JSON.stringify(v) : String(v)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <button onClick={() => setVerRegistro(null)}
                className="w-full py-2.5 bg-[#111916] border border-[#243429] text-[#b2bab4] rounded-xl text-sm font-medium hover:bg-[#162420] transition-colors">
                Cerrar
              </button>
            </div>
          </div>
