    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <BarChart3 className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              <span className="text-[#8fe0a8] tabular-nums">{opsEnPeriodo}</span> ops · {periodoActual?.subLabel}
              {ultimaSinc && (
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-md border border-[#3fb074]/40 bg-[#3fb074]/10 hover:bg-[#3fb074]/20 transition-colors text-[11.5px] font-medium text-[#8fe0a8] disabled:opacity-50"
          >
      ...
      <div className="ct-dash-wrap px-3 sm:px-6 py-5 sm:py-7 pb-24 space-y-6 sm:space-y-8 max-w-[1600px] mx-auto">
              ...
              return (
                <button
                  ...
                  className={`px-3.5 sm:px-4 py-2 text-[12.5px] sm:text-[13px] font-medium rounded-lg border transition-all tabular-nums ${
                    active
                      ? 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8] shadow-[0_0_0_3px_rgba(63,176,116,0.06)]'
        ...
        <Section title="Stock final almacenado" icon={Package} subtitle="Total fraccionado disponible">
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="relative rounded-2xl bg-gradient-to-br from-[#0e1411] via-[#111916] to-[#0e1411] border border-[#5a4820] p-6 sm:p-8 overflow-hidden">
              ...
              <div className="relative grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 items-center">
                ...
                <div className="lg:col-span-3">
                  {cargando ? (
                    <div className="space-y-3">
                      <div className="h-3 w-32 bg-[#162420] rounded animate-pulse" />
                      <div className="h-20 w-72 bg-[#162420] rounded animate-pulse" />
                      <div className="h-3 w-48 bg-[#162420] rounded animate-pulse" />
                    </div>
                  ) : (
                    <>
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center border border-[#5a4820] bg-[#C49A2C]/15">
                          <Package className="w-4 h-4 text-[#E3B94A]" strokeWidth={1.8} />
                        </div>
                        <p className="text-[10.5px] uppercase tracking-[0.18em] text-[#a7b1ab] font-semibold">
                          Stock final
                        </p>
                      </div>
                      <div className="font-display font-bold tabular-nums leading-[0.95] text-[#E3B94A] drop-shadow-[0_0_30px_rgba(227,185,74,0.15)]">
                        <span className="text-[64px] sm:text-[88px] lg:text-[100px] tracking-tight">
                          {kpis.totalKgFinal.toFixed(2)}
                        </span>
                        <span className="text-[28px] sm:text-[36px] text-[#C49A2C] font-semibold ml-2">kg</span>
                      </div>
                      <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-[12.5px] text-[#a7b1ab]">
                ...
                {stockSparkline.length > 1 && !cargando && (
                  <div className="lg:col-span-2">
                    <p className="text-[10.5px] uppercase tracking-[0.16em] text-[#5a6560] font-medium mb-2">
                      Evolución kg trim · últ. {stockSparkline.length}d
                    </p>
                    <div className="rounded-xl bg-[#0a0f0d]/40 border border-[#1a2620] p-3">
                      <SparkAreaChart
                        data={stockSparkline}
                        index="fecha"
                        categories={['kg']}
...
const TONE: Record<Tone, { bg: string; border: string; icon: string; valor: string; glow: string }> = {
  primary: { bg: 'rgba(63,176,116,0.10)',  border: '#2a5138', icon: '#6bcf8e', valor: '#e8ece9', glow: 'rgba(63,176,116,0.10)' },
  gold:    { bg: 'rgba(196,154,44,0.10)',  border: '#5a4820', icon: '#E3B94A', valor: '#E3B94A', glow: 'rgba(227,185,74,0.10)' },
}
    ...
    <section>
      <div className="flex items-end justify-between gap-3 mb-3 sm:mb-4 flex-wrap">
        <div className="flex items-center gap-2.5">
          {Icono && (
            <div className="w-7 h-7 rounded-lg flex items-center justify-center border border-[#1a2620] bg-[#0e1411]">
              <Icono className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
            </div>
          )}
          <div>
            <h2 className="font-display font-bold text-[14px] sm:text-[15px] text-[#e8ece9] tracking-tight leading-tight">{title}</h2>
            {subtitle && <p className="text-[10.5px] sm:text-[11px] text-[#5a6560] mt-0.5">{subtitle}</p>}
          </div>
        </div>
      </div>
      {children}
...
function HeroFact({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="flex items-baseline gap-1.5">
      <span className="font-mono tabular-nums font-semibold text-[15px] sm:text-[17px] text-[#d3d8d4]">{value}</span>
      <span className="text-[10.5px] sm:text-[11px] text-[#747e78]">{label}</span>
    </div>
  )
}
...
function KpiBig({
  ...
  return (
    <motion.div variants={fadeUp}>
      <div className="relative rounded-xl bg-[#0e1411] border border-[#1a2620] hover:border-[#2a5138] transition-all p-5 sm:p-6 h-full overflow-hidden group">
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-3">
            <p className="text-[10.5px] uppercase tracking-[0.16em] text-[#5a6560] font-semibold">{label}</p>
          ...
          {valor === null ? (
            <div className="h-12 w-32 bg-[#162420] rounded animate-pulse" />
          ) : (
            <div className="flex items-baseline gap-2 flex-wrap">
              ...
              {unidad && (
                <span className="text-[14px] sm:text-[16px] font-medium text-[#8e9892] tabular-nums">{unidad}</span>
              )}
            </div>
          )}
          <p className="text-[11.5px] text-[#747e78] mt-3">{hint}</p>
        </div>
      </div>
    </motion.div>
  )
...
function RatioBig({
  ...
  const fill = ok ? 'linear-gradient(90deg, #3fb074, #6bcf8e)' : 'linear-gradient(90deg, #C49A2C, #E3B94A)'
  const text = ok ? '#8fe0a8' : '#E3B94A'
  const border = ok ? '#2a5138' : '#5a4820'
  const glow = ok ? 'rgba(63,176,116,0.12)' : 'rgba(227,185,74,0.12)'
  const clamped = Math.max(0, Math.min(100, pct))
  return (
    <motion.div variants={fadeUp}>
      <div className="relative rounded-xl bg-[#0e1411] border p-5 sm:p-6 h-full overflow-hidden"
        ...
        <div className="relative">
          <h3 className="font-display font-semibold text-[13px] text-[#e8ece9] mb-3">{titulo}</h3>

          <div className="flex items-end gap-3 mb-3">
            <span className="font-display font-bold text-[44px] sm:text-[52px] leading-none tabular-nums" style={{ color: text }}>
              {pct}
            </span>
            <span className="text-[20px] sm:text-[24px] font-semibold text-[#8e9892] mb-1">%</span>
          </div>

          <div className="h-2 rounded-full bg-[#162420] overflow-hidden">
            <motion.div
              initial={{ width: 0 }}
          ...
          <div className="mt-3 space-y-1">
            <p className="text-[11.5px] text-[#a7b1ab] tabular-nums">{detalle}</p>
            <p className="text-[10.5px] text-[#5a6560]">
              Esperado: <span className="text-[#a7b1ab] tabular-nums font-mono">{esperado}</span>
            </p>
          </div>
        </div>
      </div>
    ...
    <motion.div variants={fadeUp} initial="hidden" animate="visible">
      <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5">
        <div className="flex items-start gap-2.5 mb-1">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center border border-[#2a5138] bg-[#3fb074]/10 flex-shrink-0 mt-0.5">
            <Icono className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
          </div>
          <div className="min-w-0">
            <h3 className="font-display font-semibold text-[14px] sm:text-[15px] text-[#e8ece9] tracking-tight">{titulo}</h3>
            {subtitulo && <p className="text-[10.5px] text-[#5a6560] mt-0.5">{subtitulo}</p>}
          </div>
        </div>
        {children}
      </div>
