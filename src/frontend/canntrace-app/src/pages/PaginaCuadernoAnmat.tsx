...
const CAT_COLORES: Record<EntradaCuaderno['categoria'], string> = {
  ...
  Trazabilidad: 'bg-[#3fb074]/10 text-[#6bcf8e] border-[#2a5138]',
  Mantenimiento: 'bg-violet-500/10 text-violet-300 border-violet-900/50',
  Operacion: 'bg-[#3fb074]/8 text-[#6bcf8e] border-[#243429]',
}
...
export default function PaginaCuadernoAnmat() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      {/* TopBar */}
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620] print:hidden">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <BookOpen className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Cuaderno de Campo</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">ANMAT Res 1780/2025 · {filtradas.length} actividad{filtradas.length === 1 ? '' : 'es'} {camada ? `de ${camada}` : ''}</div>
          </div>
        ...
        <div className="px-3 sm:px-6 pb-3">
          <div className="flex flex-wrap items-end gap-2 sm:gap-3">
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Camada</label>
              <select
                value={camada}
                onChange={e => setCamada(e.target.value as Camada | '')}
                className="px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] rounded-lg text-xs text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30"
              >
            ...
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Desde</label>
              <input type="date" value={desde} onChange={e => setDesde(e.target.value)}
                className="px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] rounded-lg text-xs text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Hasta</label>
              <input type="date" value={hasta} onChange={e => setHasta(e.target.value)}
                className="px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] rounded-lg text-xs text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30" />
            </div>
            <div>
              <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Categoria</label>
              <select
                value={categoria}
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#3fb074] hover:bg-[#6bcf8e] text-[#0a0f0d] rounded-lg text-xs font-semibold transition-colors"
            >
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-3 print:p-0">
        ...
        {cargando && entradas.length === 0 && (
          <div className="flex items-center justify-center py-16 text-[#5a6560] gap-2">
        ...
        {porDia.map(([dia, actividades]) => (
          <motion.div
            ...
            className="bg-[#0e1411] border border-[#1a2620] rounded-xl overflow-hidden print:break-inside-avoid"
          >
            <div className="px-4 py-2.5 border-b border-[#1a2620] flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Calendar className="w-3.5 h-3.5 text-[#3fb074]" />
                <h3 className="text-sm font-semibold text-[#e8ece9]">
                  {formatearDia(dia)}
                </h3>
              </div>
              <span className="text-[10px] text-[#5a6560] tabular-nums">{actividades.length} actividad{actividades.length === 1 ? '' : 'es'}</span>
            </div>
            <div className="divide-y divide-[#1a2620]/50">
              {actividades.map(a => {
                  ...
                  <div key={a.id} className="px-4 py-2.5 flex items-start gap-3 text-[12px]">
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-[#e8ece9]">{a.tipo}</span>
                        {a.camada && <span className="text-[10px] font-mono px-1.5 py-0.5 bg-[#111916] border border-[#243429] rounded">{a.camada}</span>}
                        {a.sistema && <span className="text-[10px] font-mono px-1.5 py-0.5 bg-[#111916] border border-[#243429] rounded">{a.sistema}</span>}
                      </div>
                      <p className="text-[#b2bab4] text-[11px]">{a.descripcion}</p>
                      {a.observaciones && <p className="text-[10px] text-[#5a6560] italic">{a.observaciones}</p>}
                    </div>
                    <div className="text-right text-[10px] text-[#5a6560] flex-shrink-0 space-y-0.5">
                      {a.responsable && <p className="font-medium text-[#8e9892]">{a.responsable}</p>}
