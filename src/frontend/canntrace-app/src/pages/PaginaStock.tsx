...
const coloresEstado: Record<string, { pill: string; dot: string }> = {
  activo:     { pill: 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8]',      dot: '#6bcf8e' },
  cuarentena: { pill: 'bg-[#E3B94A]/12 border-[#5a4820] text-[#E3B94A]',      dot: '#E3B94A' },
  baja:       { pill: 'bg-[#ff6b5a]/12 border-[#5a2820] text-[#ff6b5a]',      dot: '#ff6b5a' },
  procesado:  { pill: 'bg-[#8e9892]/12 border-[#1a2620] text-[#8e9892]',      dot: '#8e9892' },
  consumido:  { pill: 'bg-[#5a6560]/12 border-[#1a2620] text-[#747e78]',      dot: '#5a6560' },
}

const coloresSistema: Record<string, { pill: string; icon: string }> = {
  RDWC: { pill: 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8]',  icon: '#6bcf8e' },
  COCO: { pill: 'bg-[#E3B94A]/12 border-[#5a4820] text-[#E3B94A]',  icon: '#C49A2C' },
}
...
function EstadoPill({ estado }: { estado: string }) {
  const cls = coloresEstado[estado]?.pill ?? 'bg-[#5a6560]/12 border-[#1a2620] text-[#747e78]'
  return (
...
function SistemaPill({ sistema }: { sistema?: string }) {
  if (!sistema) return <span className="text-[11px] text-[#5a6560]">—</span>
  const cls = coloresSistema[sistema] ?? { pill: 'bg-[#5a6560]/10 border-[#1a2620] text-[#8e9892]', icon: '#8e9892' }
...
export default function PaginaStock() {
  ...
  const columnas = useMemo<ColumnDef<any>[]>(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2.5">
            <Icon className="w-4 h-4 text-[#5a6560] flex-shrink-0" strokeWidth={1.75} />
            <div>
              <p className="text-[12.5px] font-medium text-[#e8ece9]">{item.productos?.nombre || '-'}</p>
              <p className="text-[10px] text-[#5a6560]">{ETIQUETAS_TIPO[item.productos?.tipo_producto] || item.productos?.tipo_producto}</p>
            </div>
          </div>
        )
      },
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className="text-[12px] font-mono tabular-nums text-[#d3d8d4]">{row.original.codigo_lote}</p>
          {row.original.datos_extra?.camada && (
            <p className="text-[10px] text-[#5a6560]">Camada: {row.original.datos_extra.camada}</p>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ getValue }) => <span className="text-[12px] text-[#8e9892]">{String(getValue())}</span>,
    },
    {
      id: 'cantidad',
      header: 'Cantidad',
      ...
      cell: ({ row }) => (
        <span className="text-[12.5px] font-semibold text-[#e8ece9] tabular-nums font-mono">
          {parseFloat(row.original.cantidad).toLocaleString('es-AR')}
          <span className="font-normal text-[#5a6560] ml-1 text-[10px]">{unidadCorrecta(row.original.productos?.tipo_producto)}</span>
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: '',
      enableSorting: false,
      cell: () => <ChevronRight className="w-4 h-4 text-[#454e48]" />,
    },
  ], [])
    ...
    <>
      <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
        ...
        <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
          <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
            <Package className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
            <div className="min-w-0">
              <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Inventario</h1>
            ...
            {ultimaSinc && (
              <span className="hidden sm:inline text-[10px] text-[#5a6560] tabular-nums">
            ...
            <button onClick={exportarCSV}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#1a2620] bg-[#111916] hover:bg-[#162420] transition-colors text-[11.5px] font-medium text-[#d3d8d4]">
        ...
        <div className="px-3 sm:px-6 py-4 pb-20 space-y-3">
          ...
          {/* ── Tabs de vista + modo ── */}
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-2.5 flex flex-wrap items-center gap-2 justify-between">
                ...
                return (
                  <button key={t.v} onClick={() => setVista(t.v)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11.5px] font-medium transition-colors ${
                      active
                        ? 'bg-[#3fb074]/15 border border-[#2a5138] text-[#8fe0a8]'
                        : 'text-[#8e9892] hover:text-[#d3d8d4] hover:bg-[#111916] border border-transparent'
                    }`}>
            ...
            <div className="hidden sm:flex items-center gap-1 bg-[#0a0f0d] border border-[#1a2620] rounded-lg p-0.5">
              <button onClick={() => setModo('tabla')}
                className={`p-1.5 rounded-md transition-colors ${modo === 'tabla' ? 'bg-[#111916] text-[#8fe0a8]' : 'text-[#5a6560] hover:text-[#d3d8d4]'}`}>
                <List className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setModo('cards')}
                className={`p-1.5 rounded-md transition-colors ${modo === 'cards' ? 'bg-[#111916] text-[#8fe0a8]' : 'text-[#5a6560] hover:text-[#d3d8d4]'}`}>
          ...
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
            <div className="flex flex-wrap gap-2 p-3">
              ...
              <div className="flex-1 min-w-[200px] relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#5a6560]" />
                <input
                  type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
                  placeholder="Buscar código, producto, camada, traza…"
                  className="w-full pl-9 pr-4 py-2 bg-[#0a0f0d] border border-[#1a2620] focus:border-[#2a5138] rounded-lg text-[12px] text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none transition-colors"
                />
              </div>
              {/* Filtros toggle */}
              <button onClick={() => setFiltrosAbiertos(!filtrosAbiertos)}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[11.5px] font-medium border transition-colors ${
                  filtrosActivos > 0
                    ...
                    : 'bg-[#0a0f0d] border-[#1a2620] text-[#8e9892] hover:border-[#2a5138] hover:text-[#d3d8d4]'
                }`}>
              ...
              {filtrosActivos > 0 && (
                <button onClick={limpiarFiltros}
                  className="flex items-center gap-1 px-2 py-2 text-[11px] text-[#5a6560] hover:text-[#ff6b5a] transition-colors">
                  <X className="w-3.5 h-3.5" /> Limpiar
                </button>
              )}
              {/* Orden */}
              <div className="flex items-center gap-1 bg-[#0a0f0d] border border-[#1a2620] rounded-lg p-1 ml-auto">
                <span className="text-[10px] text-[#5a6560] px-1.5">Orden</span>
                {([
                  { v: 'fecha', label: 'Fecha' },
                  ...
                  <button key={o.v} onClick={() => { if (ordenCampo === o.v) setOrdenAsc(!ordenAsc); else setOrdenCampo(o.v) }}
                    className={`flex items-center gap-0.5 px-2 py-1 rounded-md text-[10.5px] font-medium transition-colors ${
                      ordenCampo === o.v ? 'bg-[#111916] text-[#8fe0a8]' : 'text-[#5a6560] hover:text-[#d3d8d4]'
                    }`}>
            ...
            {chipsFiltros.length > 0 && (
              <div className="border-t border-[#1a2620] py-2">
            ...
            {filtrosAbiertos && (
              <div className="border-t border-[#1a2620] p-4 bg-[#0a0f0d] space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2">Sistema</p>
                    <div className="flex gap-1.5">
                      {['todos', 'RDWC', 'COCO'].map(s => (
                        <button key={s} onClick={() => setFiltroSistema(s)}
                          className={`px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-colors ${
                            filtroSistema === s
                              ? s === 'RDWC' ? 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8]'
                              : s === 'COCO' ? 'bg-[#E3B94A]/12 border-[#5a4820] text-[#E3B94A]'
                              : 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8]'
                              : 'bg-[#111916] border-[#1a2620] text-[#8e9892] hover:border-[#2a5138]'
                          }`}>
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2">Estado</p>
                ...
                <div>
                  <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2">Tipo de producto</p>
                  <div className="flex flex-wrap gap-1.5">
                      ...
                      return (
                        <button key={t} onClick={() => toggleSet(filtroTipos, t, setFiltroTipos)}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] border transition-colors ${
                            sel ? 'bg-[#3fb074]/15 border-[#2a5138] text-[#8fe0a8]' : 'bg-[#111916] border-[#1a2620] text-[#8e9892] hover:border-[#2a5138]'
                          }`}>
                ...
                <div>
                  <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2">Camada</p>
                  <div className="flex flex-wrap gap-1.5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={f.label}>
                      <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-1.5">{f.label}</p>
                      <input type={f.type} value={f.val} onChange={e => f.set(e.target.value)}
                        placeholder={f.type === 'number' ? 'ej: 100' : ''}
                        className="w-full px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] focus:border-[#2a5138] rounded-lg text-[11.5px] text-[#d3d8d4] focus:outline-none transition-colors [color-scheme:dark]" />
                    </div>
                  ))}
                </div>
              </div>
          ...
          {cargando ? (
            <div className="text-center py-14">
              <Loader2 className="w-8 h-8 text-[#3fb074] animate-spin mx-auto" />
              <p className="mt-3 text-[12px] text-[#5a6560]">Cargando stock…</p>
            </div>
          ) : stockFiltrado.length === 0 ? (
            <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-12 text-center">
              <Package className="w-10 h-10 text-[#2a5138] mx-auto mb-3" />
              <p className="text-[#8e9892] text-[13px]">No hay lotes con esos filtros</p>
              {filtrosActivos > 0 && (
                <button onClick={limpiarFiltros} className="mt-3 text-[11.5px] text-[#8fe0a8] hover:underline">Limpiar filtros</button>
              )}
            </div>
            ...
            <div className="space-y-3">
              {porCamada.map(([cam, lotes]) => {
                ...
                return (
                  <div key={cam} className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
                    {/* Header camada */}
                    <div className="px-4 py-3 bg-[#111916] border-b border-[#1a2620] flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5">
                        <span className="font-display font-bold text-[14px] text-[#e8ece9]">{cam}</span>
                        {sistemaCam && <SistemaPill sistema={sistemaCam} />}
                      </div>
                      <span className="text-[10.5px] text-[#5a6560]">
                        {lotes.length} lotes · {totalCantidad.toFixed(0)} total
                      </span>
                    </div>
                    {/* Mobile cards */}
                    <div className="divide-y divide-[#1a2620] block sm:hidden">
                      {lotes.map((item: any) => <CardMobile key={item.id} item={item} onClick={() => setSeleccionado(item)} />)}
                    </div>
                    {/* Desktop filas */}
                    <div className="divide-y divide-[#1a2620] hidden sm:block">
                  ...
                  <>
                    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
                      <DataTable
                        ...
                        emptyState={
                          <div className="text-[#5a6560] py-6 text-center">
                            <Package className="w-7 h-7 mx-auto mb-2 text-[#2a5138]" />
                    ...
                    {stockFiltrado.length > 0 && (
                      <div className="mt-2 flex items-center justify-between px-1 text-[11px] text-[#5a6560]">
                        <span>{stockFiltrado.length} lotes</span>
                        <span className="font-semibold text-[#8e9892] tabular-nums font-mono">
                    ...
                    {stockFiltrado.map(item => {
                      ...
                      return (
                        <button key={item.id} onClick={() => setSeleccionado(item)}
                          className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-3.5 text-left hover:border-[#2a5138] hover:-translate-y-0.5 transition-all duration-200">
                          <div className="flex items-start justify-between gap-2 mb-2">
                            <div className="flex items-center gap-2">
                              <Icon className="w-4 h-4 text-[#5a6560]" strokeWidth={1.75} />
                              <p className="text-[10.5px] font-medium text-[#8e9892]">{ETIQUETAS_TIPO[item.productos?.tipo_producto] || '-'}</p>
                            </div>
                            <EstadoPill estado={item.estado} />
                          </div>
                          <p className="text-[12.5px] font-mono tabular-nums text-[#d3d8d4] truncate">{item.codigo_lote}</p>
                          <div className="flex items-center justify-between mt-2">
                            <p className="text-[22px] font-bold font-mono tabular-nums text-[#e8ece9] leading-none">
                              {parseFloat(item.cantidad).toLocaleString('es-AR')}
                              <span className="text-[10px] text-[#5a6560] ml-1 font-normal">{unidadCorrecta(item.productos?.tipo_producto)}</span>
                            </p>
                            {sistema && <SistemaPill sistema={sistema} />}
                          </div>
                          {item.datos_extra?.camada && (
                            <p className="text-[10px] text-[#5a6560] mt-2">
      ...
      <Drawer.Root open={!!seleccionado} onOpenChange={(o) => !o && setSeleccionado(null)} direction="right">
        <Drawer.Portal>
          ...
          <Drawer.Content
            className="fixed z-50 bg-[#0e1411] border-l border-[#1a2620] flex flex-col
          ...
          >
            ...
            {seleccionado && (
              <>
                {/* Drawer header */}
                <div className="flex items-center justify-between p-4 sm:p-5 border-b border-[#1a2620] sticky top-0 z-10 bg-[#0e1411]">
                  <div className="min-w-0">
                    <h3 className="font-display font-bold text-[15px] text-[#e8ece9] truncate">{seleccionado.productos?.nombre}</h3>
                    <p className="text-[11.5px] font-mono tabular-nums text-[#5a6560] truncate mt-0.5">{seleccionado.codigo_lote}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <a href={`/traza/${encodeURIComponent(seleccionado.codigo_lote)}`} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#3fb074]/40 bg-[#3fb074]/10 hover:bg-[#3fb074]/20 text-[11.5px] font-medium text-[#8fe0a8] transition-colors">
                      <ExternalLink className="w-3.5 h-3.5" />
                      CoA
                    </a>
                    <button onClick={() => setSeleccionado(null)}
                      className="p-2 hover:bg-[#111916] rounded-lg transition-colors">
                      <X className="w-5 h-5 text-[#5a6560]" />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
                  ...
                  {seleccionado.datos_extra?.sistema && (
                    <div className="p-3 rounded-xl bg-[#111916] border border-[#1a2620]">
                      <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2.5">Sistema de producción</p>
                  ...
                  {seleccionado.datos_extra && Object.keys(seleccionado.datos_extra).length > 0 && (
                    <div className="p-3 rounded-xl bg-[#111916] border border-[#1a2620]">
                      <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em] mb-2.5">Datos adicionales</p>
                      <div className="grid grid-cols-2 gap-2.5">
                        {Object.entries(seleccionado.datos_extra).map(([key, value]) => (
                          <div key={key}>
                            <p className="text-[10px] text-[#5a6560]">{key.replace(/_/g, ' ')}</p>
                            <p className="text-[12px] font-medium text-[#d3d8d4] break-words">{String(value)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <Clock className="w-3.5 h-3.5 text-[#5a6560]" />
                      <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.12em]">Historial de movimientos</p>
                    </div>
                    {cargandoMov ? (
                      <div className="flex items-center gap-2 text-[12px] text-[#5a6560] py-6 justify-center">
                        <Loader2 className="w-4 h-4 animate-spin text-[#3fb074]" /> Cargando…
                      </div>
                    ) : movimientos.length === 0 ? (
                      <div className="text-center py-6 rounded-xl bg-[#111916] border border-[#1a2620]">
                        <p className="text-[12px] text-[#5a6560]">Sin operaciones registradas</p>
                      </div>
                    ) : (
                          ...
                          return (
                            <div key={mov.id} className="flex gap-3">
                              <div className="flex flex-col items-center">
                                <div className="w-7 h-7 rounded-full bg-[#162420] border border-[#2a5138] flex items-center justify-center flex-shrink-0 z-10">
                                  <Package className="w-3.5 h-3.5 text-[#6bcf8e]" />
                                </div>
                                {!isLast && <div className="w-0.5 flex-1 min-h-[24px] bg-[#1a2620]" />}
                              </div>
                              <div className="flex-1 pb-3">
                                <div className="flex items-center justify-between gap-2">
                                  <p className="text-[12px] font-medium text-[#e8ece9]">{ETIQUETAS_OPERACION[mov.tipo_operacion as TipoOperacion] || mov.tipo_operacion}</p>
                                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${
                                    mov.estado === 'confirmada' ? 'bg-[#3fb074]/12 border-[#2a5138] text-[#8fe0a8]' :
                                    mov.estado === 'borrador'   ? 'bg-[#E3B94A]/12 border-[#5a4820] text-[#E3B94A]'
                                                                : 'bg-[#ff6b5a]/12 border-[#5a2820] text-[#ff6b5a]'
                                  }`}>{mov.estado}</span>
                                </div>
                                <div className="flex flex-wrap gap-x-3 text-[10.5px] text-[#5a6560] mt-0.5 font-mono tabular-nums">
                                ...
                                {mov.observaciones && <p className="text-[10.5px] text-[#747e78] mt-0.5 truncate">{mov.observaciones}</p>}
                              </div>
                            </div>
                          )
                        })}
...
function KpiDark({ icono: Icon, label, valor, tone }: { icono: LucideIcon; label: string; valor: string; tone: 'primary' | 'gold' }) {
  const t = tone === 'primary'
    ? { bg: 'rgba(63,176,116,0.10)', border: '#2a5138', text: '#8fe0a8' }
    : { bg: 'rgba(196,154,44,0.10)', border: '#5a4820', text: '#E3B94A' }
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-3.5 hover:border-[#2a5138] transition-colors">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium leading-tight">{label}</p>
          <p className="font-display font-bold text-[22px] sm:text-[26px] text-[#e8ece9] mt-1 leading-none tabular-nums">{valor}</p>
        </div>
...
function FilaDark({ item, onClick }: { item: any; onClick: () => void }) {
  const Icon = ICONOS_TIPO[item.productos?.tipo_producto] || Package
  return (
    <div onClick={onClick} className="flex items-center gap-3 px-4 py-2.5 hover:bg-[#111916] cursor-pointer transition-colors">
      <Icon className="w-4 h-4 text-[#5a6560] flex-shrink-0" strokeWidth={1.75} />
      <div className="flex-1 min-w-0">
        <p className="text-[12px] font-mono tabular-nums text-[#d3d8d4] truncate">{item.codigo_lote}</p>
        <p className="text-[10px] text-[#5a6560]">{ETIQUETAS_TIPO[item.productos?.tipo_producto]} · {item.instalaciones?.nombre || 'sin ubicación'}</p>
      </div>
      <span className="text-[12.5px] font-semibold font-mono tabular-nums text-[#e8ece9]">
        {parseFloat(item.cantidad).toLocaleString('es-AR')}
        <span className="text-[10px] text-[#5a6560] ml-1 font-normal">{unidadCorrecta(item.productos?.tipo_producto)}</span>
      </span>
      <EstadoPill estado={item.estado} />
      <ChevronRight className="w-4 h-4 text-[#454e48]" />
    </div>
  )
}
...
function CardMobile({ item, onClick }: { item: any; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick}
      className="w-full text-left rounded-xl bg-[#0e1411] border border-[#1a2620] p-3.5 hover:border-[#2a5138] transition-colors">
      {/* Header */}
      <div className="flex items-center justify-between gap-2 mb-2">
        <span className="font-mono tabular-nums text-[12.5px] text-[#d3d8d4] font-medium truncate">{item.codigo_lote}</span>
        <EstadoPill estado={item.estado} />
      </div>
      {/* Producto */}
      <div className="flex items-center gap-2 mb-2.5">
        <Icon className="w-3.5 h-3.5 text-[#5a6560] flex-shrink-0" strokeWidth={1.75} />
        <span className="text-[11.5px] text-[#8e9892]">{item.productos?.nombre || ETIQUETAS_TIPO[item.productos?.tipo_producto] || '-'}</span>
        {sistema && <SistemaPill sistema={sistema} />}
      </div>
      {/* Cantidad grande */}
      <div className="flex items-end justify-between gap-2">
        <span className="font-display font-bold text-[26px] text-[#e8ece9] leading-none tabular-nums font-mono">
          {parseFloat(item.cantidad).toLocaleString('es-AR')}
          <span className="text-[11px] text-[#5a6560] ml-1 font-normal">{unidadCorrecta(item.productos?.tipo_producto)}</span>
        </span>
        <div className="text-right text-[10px] text-[#5a6560] leading-tight">
...
function InfoDark({ label, valor }: { label: string; valor?: string | null }) {
  return (
    <div>
      <p className="text-[10px] text-[#5a6560] uppercase tracking-[0.10em]">{label}</p>
      <p className="text-[12.5px] font-medium text-[#d3d8d4] mt-0.5">{valor || '—'}</p>
    </div>
  )
}
