...
export default function PaginaAuditTrail() {
  ...
  function opStyle(op: string) {
    if (op === 'INSERT') return 'bg-[#3fb074]/15 text-[#8fe0a8] border border-[#2a5138]'
    if (op === 'UPDATE') return 'bg-[#C49A2C]/15 text-[#E3B94A] border border-[#5a4820]'
    if (op === 'DELETE') return 'bg-[#7a2820]/30 text-[#ff8a7a] border border-[#7a2820]'
    return 'bg-[#111916] text-[#b2bab4] border border-[#1a2620]'
  }
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <ShieldCheck className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Audit Trail</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              {datos?.lote?.codigo_lote
                ? <span className="font-mono tabular-nums text-[#b2bab4]">{datos.lote.codigo_lote}</span>
                : 'Cargando...'}
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span> cadena hash SHA-256</span>
            </div>
          </div>
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-[#3fb074]/30 bg-[#3fb074]/10 text-[#8fe0a8] text-[10.5px] uppercase tracking-widest font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-[#6bcf8e] shadow-[0_0_6px_rgba(107,207,142,0.8)]" />GAMP5
          </span>
        </div>
      </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        <Link
          to="/trazabilidad"
          className="inline-flex items-center gap-2 text-[11.5px] text-[#5a6560] hover:text-[#8fe0a8] font-medium transition-colors"
        >
        ...
        {cargando && (
          <div className="space-y-4">
            <div className="h-24 rounded-xl bg-[#0e1411] border border-[#1a2620] animate-pulse" />
            <KpiGridSkeleton count={4} />
            <div className="h-64 rounded-xl bg-[#0e1411] border border-[#1a2620] animate-pulse" />
          </div>
        )}
        ...
        {datos && (
          ...
          >
            ...
            <motion.div
              variants={fadeUp}
              className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5"
            >
              <div className="flex items-start gap-4 flex-wrap">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="w-12 h-12 rounded-xl bg-[#3fb074]/10 flex items-center justify-center flex-shrink-0">
                    <ShieldCheck className="w-6 h-6 text-[#3fb074]" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium">Lote</p>
                    <p className="font-display text-[18px] font-bold text-[#e8ece9] font-mono tabular-nums break-all">{datos.lote.codigo_lote}</p>
                  </div>
                </div>
            ...
            {datos.operaciones.length > 0 && (
              <motion.section
                ...
                className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5"
              >
                <h3 className="text-[13px] font-display font-bold text-[#e8ece9] mb-3 flex items-center gap-2">
                  <Activity className="w-4 h-4 text-[#3fb074]" />
                  Operaciones seed-to-sale
                  <span className="text-[11px] font-normal text-[#5a6560]">({datos.operaciones.length})</span>
                </h3>
                <ul className="divide-y divide-[#1a2620]">
                  {datos.operaciones.map((o) => (
                    <li key={o.id} className="flex items-center gap-3 py-2 text-[11.5px]">
                      ...
                      <span className="text-[#8e9892] tabular-nums flex-shrink-0">
                        {new Date(o.fecha_operacion).toLocaleDateString('es-AR')}
                      </span>
                      <span className="text-[#d3d8d4] font-mono tabular-nums">
                        {o.cantidad_entrada ? `+${o.cantidad_entrada}` : ''}
                        {o.cantidad_salida ? ` -${o.cantidad_salida}` : ''}
                      </span>
                      {o.responsable && <span className="text-[#5a6560] truncate hidden sm:inline">· {o.responsable}</span>}
                      {o.estado === 'confirmado' && <CheckCircle2 className="w-3.5 h-3.5 text-[#3fb074] ml-auto flex-shrink-0" aria-label="Confirmada" />}
                    </li>
                  ))}
                </ul>
              </motion.section>
            ...
            >
              <h3 className="text-[13px] font-display font-bold text-[#e8ece9] mb-3 flex items-center gap-2">
                <Hash className="w-4 h-4 text-[#3fb074]" />
                Historial de cambios
                <span className="text-[11px] font-normal text-[#5a6560]">({datos.registros.length})</span>
              </h3>
              {datos.registros.length === 0 ? (
                ...
                <div className="space-y-2">
                  {datos.registros.map((r) => (
                    <div
                      key={r.id}
                      className="border border-[#1a2620] rounded-lg overflow-hidden bg-[#111916]"
                    >
                      <button
                        onClick={() => setExpandido(expandido === r.id ? null : r.id)}
                        className="w-full flex items-center gap-3 px-3 py-2 text-[11.5px] hover:bg-[#162420] transition-colors"
                      ...
                      >
                        ...
                        <span className="text-[#d3d8d4] font-mono tabular-nums flex-shrink-0 hidden sm:inline">{r.tabla_nombre}</span>
                        <span className="text-[#8e9892] tabular-nums flex-shrink-0">{new Date(r.fecha).toLocaleString('es-AR', { dateStyle: 'short', timeStyle: 'short' })}</span>
                        <span className="text-[#5a6560] truncate flex-1 text-left hidden md:inline">{r.usuario_email}</span>
                        <span className="text-[10px] text-[#6bcf8e] font-mono tabular-nums flex-shrink-0">{r.hash_sha256?.slice(0, 8)}…{r.hash_sha256?.slice(-4)}</span>
                      </button>
                      {expandido === r.id && (
                        <motion.div
                          ...
                          className="bg-[#0a0f0d] p-3 text-[11px] space-y-2 border-t border-[#1a2620]"
                        >
                          <div>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Hash SHA-256</p>
                            <p className="font-mono text-[10px] text-[#6bcf8e] break-all">{r.hash_sha256}</p>
                          </div>
                          <div>
                            <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Hash anterior (encadenado)</p>
                            <p className="font-mono text-[10px] text-[#747e78] break-all">{r.hash_anterior || '(primer registro de la cadena)'}</p>
                          </div>
                          {r.campos_modificados && Array.isArray(r.campos_modificados) && r.campos_modificados.length > 0 && (
                            <div>
                              <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1">Campos modificados</p>
                              <div className="flex flex-wrap gap-1">
                                {r.campos_modificados.map((c: string) => (
                                  <span key={c} className="px-2 py-0.5 bg-[#C49A2C]/15 text-[#E3B94A] border border-[#5a4820] rounded text-[10px] font-mono">{c}</span>
                                ))}
                              </div>
                            </div>
                          )}
                          {r.datos_nuevos && (
                            <details>
                              <summary className="cursor-pointer text-[#5a6560] hover:text-[#b2bab4] font-medium text-[10.5px]">
                                Ver JSON completo
                              </summary>
                              <pre className="mt-2 p-2 bg-[#060908] rounded text-[10px] text-[#8fe0a8] overflow-x-auto max-h-60">{JSON.stringify(r.datos_nuevos, null, 2)}</pre>
                            </details>
                          )}
                          {r.ip_origen && (
                            <p className="text-[#5a6560] flex items-center gap-1 text-[10.5px]">
                              IP: <code className="font-mono text-[#b2bab4]">{r.ip_origen}</code>
                            </p>
                          )}
                        </motion.div>
                      )}
            ...
            <motion.div
              variants={fadeUp}
              className="rounded-xl bg-[#3fb074]/8 border border-[#2a5138] p-4 flex items-start gap-3"
            >
              <Lock className="w-4 h-4 text-[#3fb074] flex-shrink-0 mt-0.5" />
              <p className="text-[11px] text-[#b2bab4] leading-relaxed">
                Este audit trail cumple con <strong className="text-[#8fe0a8]">21 CFR Part 11</strong> (FDA),{' '}
                <strong className="text-[#8fe0a8]">EU-GMP Annex 11</strong> (EMA) y{' '}
                <strong className="text-[#8fe0a8]">ALCOA+</strong>. Registros append-only con hash SHA-256 encadenado
...
function InfoMini({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium">{label}</p>
      <p className={`text-[12.5px] font-medium text-[#d3d8d4] ${mono ? 'font-mono tabular-nums' : ''}`}>
        {value}
      </p>
    </div>
  )
...
function HashChainBadge({ valido }: { valido: boolean }) {
  if (valido) {
    return (
      <div className="rounded-xl p-4 bg-[#3fb074]/8 border border-[#2a5138] flex items-start gap-3">
        <div className="w-9 h-9 rounded-full bg-[#3fb074]/20 flex items-center justify-center flex-shrink-0">
          <Lock className="w-4 h-4 text-[#6bcf8e]" />
        </div>
        <div className="min-w-0">
  ...
  return (
    <div className="rounded-xl p-4 bg-[#7a2820]/20 border border-[#7a2820] flex items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[11px] text-[#8e9892] mt-0.5">
...
function Stat({ icon: Icon, label, value, color }: {
  ...
  const styles: Record<typeof color, { icon: string; bg: string }> = {
    primary: { icon: 'text-[#3fb074]', bg: 'bg-[#3fb074]/10' },
    gold:    { icon: 'text-[#E3B94A]', bg: 'bg-[#C49A2C]/10' },
    neutral: { icon: 'text-[#8e9892]', bg: 'bg-[#111916]' },
  }
  ...
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
      ...
      <p className="text-[18px] font-bold font-display text-[#e8ece9] tabular-nums">{value}</p>
    </div>
  )
}
