    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <ClipboardList className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Registros CUMCS</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">{tipos.length} planillas · 10 grupos · <span className="hidden sm:inline">conservacion 7 años</span></div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          ...
          <select
            ...
            className="px-3 py-2 bg-[#0e1411] border border-[#1a2620] rounded-lg text-[13px] text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138] cursor-pointer"
          >
        ...
        {(areaFiltro !== 'todas' || grupoFiltro !== 'todos') && !busqueda && (
          <div className="flex items-center gap-2 text-xs text-[#5a6560]">
            <button onClick={() => { setAreaFiltro('todas'); setGrupoFiltro('todos') }} className="hover:text-[#3fb074] transition-colors font-medium">
              Todas las areas
            </button>
            {areaFiltro !== 'todas' && (() => {
              ...
              return a ? <>
                ...
                <button onClick={() => setGrupoFiltro('todos')} className="hover:text-[#3fb074] transition-colors font-medium">{a.nombre}</button>
              </> : null
            })()}
            {grupoFiltro !== 'todos' && (
              <>
                <ChevronRight className="w-3 h-3" />
                <span className="font-mono text-[#8e9892]">{grupoFiltro}</span>
              </>
            )}
          </div>
        )}
            ...
            {AREAS.map(area => {
              ...
              return (
                <button
                  ...
                  className="group p-4 sm:p-5 rounded-xl border border-[#1a2620] bg-[#0e1411] text-left transition-all duration-200 hover:border-[#2a5138] hover:bg-[#111916]"
                >
                  <div className="flex items-center gap-3 mb-2">
                    {Icon && <Icon className="w-5 h-5 text-[#3fb074] transition-transform group-hover:scale-110" strokeWidth={1.5} />}
                    <span className="text-[10px] font-semibold text-[#5a6560] tracking-widest uppercase">{area.grupos.length} grupos</span>
                  </div>
                  <p className="text-sm font-semibold text-[#e8ece9]">{area.nombre}</p>
                  <p className="text-[11px] text-[#8e9892] mt-1 leading-snug">{area.descripcion}</p>
                  <p className="text-[10px] text-[#5a6560] mt-2">{count} registros</p>
                </button>
              )
            })}
          </div>
              ...
              {area.grupos.map(g => {
                ...
                return (
                  <button
                    ...
                    className="group p-4 rounded-xl border border-[#1a2620] bg-[#0e1411] text-center transition-all duration-200 hover:border-[#2a5138] hover:bg-[#111916]"
                  >
                    <div className="flex justify-center mb-2 transition-transform group-hover:scale-110">
                      {Icon && <Icon className="w-5 h-5 text-[#3fb074]" strokeWidth={1.75} />}
                    </div>
                    <p className="text-xs font-semibold text-[#e8ece9] tracking-wide">{g}</p>
                    <p className="text-[10px] text-[#8e9892] mt-1 leading-tight line-clamp-2">{nombre}</p>
                    <p className="text-[10px] text-[#5a6560] mt-1">{count} reg · {obligatorios} oblig</p>
                  </button>
                )
              })}
            </div>
        ...
        {cargando ? (
          <div className="text-center py-12">
            ...
            <p className="mt-3 text-sm text-[#5a6560]">Cargando registros CUMCS...</p>
          </div>
        ) : !mostrarLista ? null : (
          ...
          Object.entries(porGrupo).map(([grupo, registros]) => {
            ...
            const renderItem = (reg: TipoRegistro, last: boolean) => (
              <div
                key={reg.codigo}
                className={`flex items-center gap-3 p-3 sm:p-4 hover:bg-[#162420] cursor-pointer transition-colors ${last ? '' : 'border-b border-[#1a2620]'}`}
                onClick={() => { limpiarForm(); setMsgExito(''); setMsgError(''); setRegistroActivo(reg) }}
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#e8ece9] truncate">{reg.nombre}</p>
                  <p className="text-[11px] text-[#5a6560] hidden sm:block">{reg.conservacion}</p>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {reg.requiere_firma && <span className="text-[9px] font-medium border border-[#243429] text-[#6bcf8e] px-1.5 py-0.5 rounded hidden sm:inline">FIRMA</span>}
                </div>
                <ChevronRight className="w-4 h-4 text-[#5a6560] flex-shrink-0" />
              </div>
            )
            ...
            return (
              <div key={grupo}>
                <div className="flex items-center gap-2 mb-2">
                  {Icon && <Icon className="w-4 h-4 text-[#3fb074]" strokeWidth={1.75} />}
                  <h3 className="text-sm font-semibold text-[#e8ece9]">{registros[0]?.grupo_nombre}</h3>
                  <span className="text-xs text-[#5a6560]">({registros.length})</span>
                </div>

                {busqueda || porSub.length === 0 ? (
                  <div className="bg-[#0e1411] rounded-xl border border-[#1a2620] overflow-hidden">
                  ...
                  <div className="space-y-3">
                    {porSub.map(sub => (
                      <div key={sub.nombre}>
                        <p className="text-[10px] font-semibold text-[#5a6560] uppercase tracking-[0.14em] mb-1.5 pl-1">{sub.nombre}</p>
                        <div className="bg-[#0e1411] rounded-xl border border-[#1a2620] overflow-hidden">
      ...
      {registroActivo && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" onClick={() => setRegistroActivo(null)}>
          <div className="bg-[#0e1411] border border-[#1a2620] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="px-5 py-4 border-b border-[#1a2620] flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h3 className="text-base font-display font-bold text-[#e8ece9]">{registroActivo.nombre}</h3>
                <p className="text-[11px] text-[#5a6560] mt-0.5">
                  {registroActivo.grupo_nombre}
                </p>
              </div>
              <button onClick={() => setRegistroActivo(null)} className="p-1.5 rounded-lg hover:bg-[#162420] text-[#5a6560] hover:text-[#d3d8d4] transition-colors flex-shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              {msgExito && <div className="p-3 bg-[#3fb074]/10 border border-[#2a5138] text-[#6bcf8e] rounded-xl text-sm font-medium">{msgExito}</div>}
              {msgError && <div className="p-3 bg-[#7a2820]/20 border border-[#7a2820]/50 text-[#ff8a7a] rounded-xl text-sm font-medium">{msgError}</div>}

              <div>
                <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Fecha</label>
                <input type="date" value={formFecha} onChange={e => setFormFecha(e.target.value)}
                  className="w-full px-3 py-2.5 bg-[#111916] border border-[#243429] rounded-lg text-sm text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138]" />
              </div>

              <div>
                <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Responsable *</label>
                <input type="text" value={formResponsable} onChange={e => setFormResponsable(e.target.value)} placeholder="Nombre del responsable"
                  className="w-full px-3 py-2.5 bg-[#111916] border border-[#243429] rounded-lg text-sm text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138]" />
              </div>
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Observaciones</label>
                <textarea rows={3} placeholder="Observaciones adicionales..." value={formObs} onChange={e => setFormObs(e.target.value)}
                  className="w-full px-3 py-2.5 bg-[#111916] border border-[#243429] rounded-lg text-sm text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138] resize-none" />
              </div>

              {/* Info GAMP5 */}
              <div className="p-3 bg-[#111916] border border-[#1a2620] rounded-xl text-[11px] text-[#5a6560] space-y-1">
                ...
                <p className="text-[#3fb074]/60">Audit trail inmutable + firma SHA-256</p>
              </div>
              ...
              <div className="flex gap-3 pt-1">
                <button
                  ...
                  className="flex-1 py-2.5 bg-[#111916] border border-[#243429] text-[#b2bab4] rounded-xl text-sm font-medium hover:bg-[#162420] transition-colors"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={guardando}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#3fb074] text-[#0a0f0d] rounded-xl text-sm font-semibold hover:bg-[#6bcf8e] disabled:opacity-50 transition-colors"
                >
                  {guardando ? (
