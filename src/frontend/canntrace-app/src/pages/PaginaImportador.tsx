    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <FileSpreadsheet className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Importador de Datos</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              Carga masiva desde Excel
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span>.xlsx</span>
            </div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        {/* Config */}
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Destino</p>
              <div className="relative">
                <select
                  value={tabla}
                  onChange={e => { setTabla(e.target.value); setPreview(null); setArchivo(null) }}
                  className="appearance-none pl-3 pr-8 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] rounded-lg text-[12px] text-[#d3d8d4] outline-none cursor-pointer transition-colors"
                >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] rounded-lg text-[12px] text-[#a7b1ab] transition-colors"
            >
              <Download className="w-3.5 h-3.5" strokeWidth={1.8} />
              <span>Plantilla</span>
            </button>

            <label className="ml-auto cursor-pointer flex items-center gap-1.5 px-4 py-2 bg-[#3fb074]/15 hover:bg-[#3fb074]/25 border border-[#2a5138] rounded-lg text-[12px] font-medium text-[#8fe0a8] transition-colors">
          ...
          <div className="rounded-lg bg-[#111916] border border-[#1a2620] px-3 py-2">
            <p className="text-[10px] text-[#5a6560]">
              <span className="text-[#8e9892] font-medium">Columnas esperadas:</span>{' '}
              <span className="font-mono text-[#6bcf8e]">{tSel.columnas.join(', ')}</span>
            </p>
          </div>
        </div>
        ...
        {!preview && !procesando && (
          <label className="flex flex-col items-center justify-center gap-3 px-6 py-12 rounded-xl border-2 border-dashed border-[#1a2620] hover:border-[#2a5138] bg-[#0e1411] cursor-pointer transition-colors">
            <Upload className="w-8 h-8 text-[#2a5138]" strokeWidth={1.5} />
            <div className="text-center">
              <p className="text-[13px] font-medium text-[#d3d8d4]">Click o arrastra el archivo Excel</p>
              <p className="text-[11px] text-[#5a6560] mt-0.5">.xlsx · .xls</p>
            </div>
            <input type="file" accept=".xlsx,.xls" className="hidden" disabled={procesando}
              onChange={e => e.target.files?.[0] && leerArchivo(e.target.files[0])} />
          </label>
        ...
        {procesando && !preview && (
          <div className="flex items-center justify-center gap-2 py-10 text-[#5a6560]">
            <Loader2 className="w-5 h-5 animate-spin text-[#3fb074]" />
        ...
        {preview && (
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
            <div className="px-4 py-3 border-b border-[#1a2620] flex items-center justify-between gap-2 flex-wrap">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <FileSpreadsheet className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
                ...
                <p className="text-[10.5px] text-[#5a6560] mt-0.5">{archivo?.name}</p>
              </div>
              <button
                onClick={importar}
                disabled={procesando}
            ...
            <div className="overflow-x-auto">
                ...
                <thead>
                  <tr className="border-b border-[#1a2620]">
                    {preview.columnas.map(c => (
                      <th key={c} className="px-3 py-2 text-left font-mono text-[#6bcf8e] text-[10px] whitespace-nowrap">{c}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {preview.filas.map((f, i) => (
                    <tr key={i} className="border-b border-[#1a2620] last:border-0 hover:bg-[#111916] transition-colors">
                      {preview.columnas.map(c => (
                        <td key={c} className="px-3 py-2 text-[#a7b1ab] font-mono whitespace-nowrap truncate max-w-[180px]">
                          {String(f[c] ?? '')}
                        </td>
                      ))}
                    </tr>
              ...
              {preview.total > 20 && (
                <p className="px-3 py-2 text-[10.5px] text-[#5a6560] border-t border-[#1a2620]">
        ...
        {resultado && (
          <div className={`rounded-xl border p-4 ${
            resultado.err === 0
              ? 'bg-[#3fb074]/08 border-[#2a5138]'
              : 'bg-[#C49A2C]/08 border-[#5a4820]'
          }`}>
            <div className="flex items-center gap-2 mb-3">
              {resultado.err === 0
                ? <CheckCircle2 className="w-4 h-4 text-[#6bcf8e]" strokeWidth={1.8} />
                : <AlertCircle className="w-4 h-4 text-[#E3B94A]" strokeWidth={1.8} />}
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">Resultado de importacion</h3>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg bg-[#3fb074]/10 border border-[#2a5138] p-3">
                <p className="text-[10px] uppercase tracking-[0.12em] text-[#6bcf8e] font-medium">Filas OK</p>
                <p className="font-display font-bold text-[22px] tabular-nums text-[#8fe0a8]">{resultado.ok}</p>
              </div>
            ...
            {resultado.errores.length > 0 && (
              <div className="mt-3 space-y-1">
                <p className="text-[10px] text-[#8e9892] font-semibold">Errores (primeros 10):</p>
