...
const iconoEstado: Record<string, { icono: typeof CheckCircle2; color: string; bg: string; border: string; label: string }> = {
  confirmada: { icono: CheckCircle2, color: '#6bcf8e', bg: 'rgba(63,176,116,0.10)', border: '#2a5138', label: 'Confirmada' },
  borrador:   { icono: Clock,         color: '#E3B94A', bg: 'rgba(196,154,44,0.10)',  border: '#5a4820', label: 'Borrador' },
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <History className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Historial de Operaciones</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              <span className="tabular-nums">{operaciones.length}</span> operaciones
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span>audit trail SHA-256</span>
            </div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="flex-1 min-w-[180px] relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#5a6560]" strokeWidth={1.8} />
            <input
              type="text"
              value={busqueda}
              onChange={(e) => setBusqueda(e.target.value)}
              placeholder="Buscar lote, responsable…"
              className="w-full pl-9 pr-4 py-2 bg-[#0e1411] border border-[#1a2620] hover:border-[#243429] focus:border-[#2a5138] rounded-lg text-[12px] text-[#d3d8d4] placeholder-[#5a6560] outline-none transition-colors"
            />
          </div>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2 bg-[#0e1411] border border-[#1a2620] hover:border-[#243429] rounded-lg text-[12px] text-[#d3d8d4] outline-none cursor-pointer transition-colors"
            >
          ...
          {/* Export */}
          <button className="flex items-center gap-1.5 px-3 py-2 bg-[#3fb074]/15 border border-[#2a5138] hover:bg-[#3fb074]/25 rounded-lg text-[12px] font-medium text-[#8fe0a8] transition-colors">
        ...
        {cargando ? (
          <div className="space-y-2">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="h-16 bg-[#0e1411] border border-[#1a2620] rounded-xl animate-pulse" />
            ))}
          </div>
          ...
          <>
            {/* Desktop: tabla */}
            <div className="hidden sm:block rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {operacionesFiltradas.map((op) => {
                    ...
                    return (
                      <tr key={op.id} className="border-b border-[#1a2620] last:border-0 hover:bg-[#111916] transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <span className="text-[16px] leading-none" aria-hidden="true">
                              {ICONOS_OPERACION[op.tipo_operacion as TipoOperacion] || '?'}
                            </span>
                            <span className="font-medium text-[#e8ece9]">
                              {ETIQUETAS_OPERACION[op.tipo_operacion as TipoOperacion] || op.tipo_operacion}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3 font-mono text-[#a7b1ab] tabular-nums">{lote}</td>
                        <td className="px-4 py-3 text-[#a7b1ab] tabular-nums">{cantidad}</td>
                        <td className="px-4 py-3">
            ...
            <div className="sm:hidden space-y-2">
              {operacionesFiltradas.map((op) => {
                ...
                const cantidad = op.cantidad_entrada || op.cantidad_salida || '-'
                return (
                  <div key={op.id} className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 justify-between">
                          <p className="text-[12px] font-medium text-[#e8ece9] truncate">
                            {ETIQUETAS_OPERACION[op.tipo_operacion as TipoOperacion] || op.tipo_operacion}
                          </p>
                        ...
                        <p className="text-[10.5px] text-[#747e78] font-mono tabular-nums mt-0.5">
                          {lote} · {cantidad}
                        </p>
                        {op.observaciones && (
                          <p className="text-[10.5px] text-[#5a6560] mt-0.5 truncate">{op.observaciones}</p>
                        )}
                        <p className="text-[10px] text-[#5a6560] mt-1 font-mono tabular-nums">
