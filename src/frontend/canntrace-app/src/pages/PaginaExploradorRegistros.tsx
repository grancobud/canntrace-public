    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <Database className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Explorador de Registros</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">Vista admin · edicion + firma 21 CFR Part 11</div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium whitespace-nowrap transition-colors flex-shrink-0 ${
                    activa
                      ? 'bg-[#3fb074]/15 border border-[#2a5138] text-[#6bcf8e]'
                      : 'text-[#5a6560] hover:text-[#b2bab4] hover:bg-[#111916]'
                  }`}
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#3fb074]/15 border border-[#2a5138] flex items-center justify-center">
              <tablaSel.icon className="w-5 h-5 text-[#3fb074]" strokeWidth={1.5} />
            </div>
            <div>
              <h2 className="text-base font-display font-bold text-[#e8ece9]">{tablaSel.label}</h2>
              <p className="text-[11px] text-[#5a6560] font-mono">
                {tablaSel.key} · {tablaSel.grupo}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <div className="px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] rounded-lg text-xs">
              <span className="text-[#5a6560]">Total: </span>
              <span className="font-bold text-[#e8ece9] tabular-nums">{total.toLocaleString('es-AR')}</span>
            </div>
            <button
              onClick={() => void cargar()}
              disabled={cargando}
              className="p-2 bg-[#0e1411] border border-[#1a2620] hover:border-[#2a5138] rounded-lg transition-colors disabled:opacity-50"
              title="Refrescar"
            >
              <RefreshCcw className={`w-3.5 h-3.5 text-[#5a6560] ${cargando ? 'animate-spin' : ''}`} />
            </button>
            <button
              onClick={() => setMostrarBusqueda(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#C49A2C]/15 border border-[#5a4820] text-[#E3B94A] rounded-lg text-xs font-medium hover:bg-[#C49A2C]/20 transition-colors"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0e1411] border border-[#1a2620] hover:border-[#2a5138] rounded-lg text-xs text-[#b2bab4] transition-colors disabled:opacity-40"
              title="Exportar CSV"
            >
        ...
        {total > 500 && !error && (
          <div className="p-2.5 bg-[#5a4820]/20 border border-[#5a4820]/50 rounded-lg text-[11px] text-[#E3B94A]">
            Mostrando las 500 mas recientes de {total.toLocaleString('es-AR')}. Usa el buscador para filtrar.
          </div>
        )}

        {/* Tabla */}
        <div className="bg-[#0e1411] border border-[#1a2620] rounded-xl overflow-hidden">
          <DataTable
            data={datos}
            columns={columnas}
            busqueda
            ...
            emptyState={
              cargando ? (
                <div className="flex items-center justify-center gap-2 py-12 text-[#5a6560]">
                  <RefreshCcw className="w-4 h-4 animate-spin" /> Cargando registros...
                </div>
              ) : (
                <div className="text-center py-12 text-[#5a6560]">
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl max-h-[90vh] bg-[#0e1411] rounded-t-2xl sm:rounded-2xl border border-[#1a2620] overflow-hidden flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-5 py-4 border-b border-[#1a2620] flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-lg bg-[#3fb074]/15 border border-[#2a5138] flex items-center justify-center flex-shrink-0">
              <tabla.icon className="w-4 h-4 text-[#3fb074]" strokeWidth={1.75} />
            </div>
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-[#e8ece9] truncate">
                {tabla.label}
                {typeof registro.tipo === 'string' && <span className="ml-2 text-[11px] font-mono text-[#3fb074]">{registro.tipo}</span>}
              </h3>
              <p className="text-[10px] text-[#5a6560] font-mono truncate">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {!editando ? (
              <>
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#C49A2C]/15 border border-[#5a4820] text-[#E3B94A] rounded-lg text-xs font-semibold transition-colors hover:bg-[#C49A2C]/25"
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#111916] border border-[#243429] rounded-lg text-xs text-[#b2bab4] hover:border-[#2a5138] transition-colors"
                >
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs text-[#5a6560] hover:text-[#ff8a7a] transition-colors"
              >
                Cancelar
              </button>
            )}
            <button
              onClick={onClose}
              className="p-1.5 text-[#5a6560] hover:text-[#d3d8d4] transition-colors"
              aria-label="Cerrar"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div className="space-y-0">
            {camposOrdenados.map(k => {
              ...
              return (
                <div key={k} className="grid grid-cols-[130px_1fr] gap-3 items-start py-1.5 border-b border-[#1a2620]/60">
                  <div className="text-[11px] text-[#5a6560] pt-1.5 flex items-center gap-1">
                    {prettyLabel(k)}
                    {cambiado && <span className="w-1.5 h-1.5 rounded-full bg-[#E3B94A]" title="Modificado" />}
                  </div>
                  <div className="min-w-0">
                    {readonly ? (
                      <div className="text-[12px] text-[#d3d8d4] font-mono break-all pt-1.5">
                        {renderValor(v)}
                      </div>
                    ) : (
                      <input
                        ...
                        className="w-full px-2.5 py-1.5 bg-[#111916] border border-[#243429] rounded-lg text-[12px] text-[#d3d8d4] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138] font-mono"
                      />
                    )}
                  </div>
                </div>
          ...
          {registro.datos_extra != null && typeof registro.datos_extra === 'object' && (
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-2">datos_extra</p>
              <pre className="text-[10px] bg-[#0a0f0d] border border-[#1a2620] rounded-lg p-3 overflow-x-auto font-mono max-h-64 overflow-y-auto text-[#8e9892]">
        ...
        {/* Footer guardar */}
        {editando && (
          <div className="px-5 py-3 border-t border-[#1a2620] flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#3fb074] hover:bg-[#6bcf8e] disabled:opacity-50 text-[#0a0f0d] rounded-lg text-sm font-semibold transition-colors"
            >
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#0e1411] border border-[#1a2620] rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-3 p-4 border-b border-[#1a2620]">
          <div className="w-9 h-9 rounded-xl bg-[#C49A2C]/15 border border-[#5a4820] flex items-center justify-center">
            <Sparkles className="w-4 h-4 text-[#E3B94A]" strokeWidth={1.75} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-display font-bold text-[#e8ece9]">Busqueda inteligente</h3>
            <p className="text-[11px] text-[#5a6560]">Full-text castellano sobre todos los registros (pgroonga)</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-[#111916] rounded-lg transition-colors">
            <X className="w-4 h-4 text-[#5a6560]" />
          </button>
        </div>

        {/* Input */}
        <form onSubmit={buscar} className="p-4 border-b border-[#1a2620]">
          <div className="relative">
            ...
            <input
              ...
              autoFocus
              className="w-full pl-9 pr-24 py-2.5 bg-[#111916] border border-[#243429] rounded-lg text-sm text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none focus:ring-1 focus:ring-[#3fb074]/30 focus:border-[#2a5138]"
            />
            <button
              type="submit"
              disabled={buscando || !q.trim()}
              className="absolute right-1.5 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-[#3fb074] text-[#0a0f0d] rounded-md text-xs font-semibold hover:bg-[#6bcf8e] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-2">
          ...
          {resultados.map((r, i) => (
            <button
              ...
              className="w-full text-left p-3 m-1 bg-[#111916] border border-[#243429] rounded-lg hover:border-[#2a5138] transition-colors"
            >
              <div className="flex items-start justify-between gap-3 mb-1.5">
                <span className="inline-flex items-center px-2 py-0.5 bg-[#3fb074]/10 text-[#6bcf8e] rounded-md text-[10px] font-mono font-medium">
                  {r.origen}
                </span>
                <span className="text-[10px] text-[#5a6560] font-mono tabular-nums flex-shrink-0">
                  rank {r.rank.toFixed(3)}
                </span>
              </div>
              <p className="text-xs text-[#b2bab4] leading-relaxed line-clamp-2">
                {r.resumen}
              </p>
            </button>
          ))}
