    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <FlaskConical className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">CoA Parser</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              Parser PDF laboratorio
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span>cannabinoides · terpenos · controles</span>
            </div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <motion.div
          ...
          className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4"
        >
          <div className="flex items-center gap-3 mb-4">
            <div className="w-9 h-9 rounded-lg bg-[#3fb074]/10 border border-[#2a5138] flex items-center justify-center flex-shrink-0">
              <FlaskConical className="w-4 h-4 text-[#6bcf8e]" strokeWidth={1.8} />
            </div>
            <div>
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">Certificado de analisis</h3>
              <p className="text-[10.5px] text-[#747e78]">PDF emitido por laboratorio acreditado</p>
            </div>
          </div>

          <label
            htmlFor="coa-file"
            className="flex flex-col items-center justify-center gap-2 px-6 py-10 border-2 border-dashed border-[#1a2620] hover:border-[#2a5138] rounded-xl cursor-pointer hover:bg-[#3fb074]/05 transition-colors"
          >
            {procesando ? (
              <>
                <Loader2 className="w-7 h-7 text-[#3fb074] animate-spin" />
                <p className="text-[12px] text-[#d3d8d4] font-semibold">Procesando {fileName}...</p>
                <p className="text-[10.5px] text-[#5a6560]">Extrayendo texto + parseando</p>
              </>
            ) : (
              <>
                <Upload className="w-7 h-7 text-[#2a5138]" strokeWidth={1.5} />
                <p className="text-[12px] text-[#d3d8d4] font-semibold">Click para subir PDF</p>
                <p className="text-[10.5px] text-[#5a6560]">o arrastra aqui</p>
                {fileName && <p className="text-[10.5px] text-[#6bcf8e] mt-1">Ultimo: {fileName}</p>}
              </>
            )}
            <input
              ref={inputRef}
        ...
        {hayData && (
          ...
          >
            ...
            <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4 space-y-3">
              <div>
                <p className="font-display font-semibold text-[13px] text-[#e8ece9]">Guardar en resultados_laboratorio</p>
                <p className="text-[10.5px] text-[#747e78] mt-0.5">Asocia el CoA a un lote para la traza publica y auditoria.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  <select
                    ...
                    className="appearance-none w-full pl-3 pr-8 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] rounded-lg text-[12px] text-[#d3d8d4] font-mono tabular-nums outline-none cursor-pointer transition-colors"
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 px-4 py-2 bg-[#3fb074]/15 hover:bg-[#3fb074]/25 border border-[#2a5138] text-[#8fe0a8] rounded-lg text-[12px] font-semibold transition-colors disabled:opacity-50"
                >
                ...
                <button
                  onClick={copiarJSON}
                  className="flex items-center justify-center gap-1.5 px-3 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] text-[#a7b1ab] rounded-lg text-[12px] font-semibold transition-colors"
                  title="Copiar JSON"
                >
                  {copiado ? <Check className="w-4 h-4 text-[#6bcf8e]" /> : <Copy className="w-4 h-4" />}
                </button>
              </div>
              {lotes.length === 0 && (
                <p className="text-[10.5px] text-[#E3B94A]">
            ...
            <div className="flex items-start gap-3 rounded-xl bg-[#C49A2C]/08 border border-[#5a4820] p-3">
              <AlertCircle className="w-4 h-4 text-[#E3B94A] flex-shrink-0 mt-0.5" strokeWidth={1.8} />
              <p className="text-[11px] text-[#a7b1ab]">
...
const TONE_STYLES: Record<Tone, { item_bg: string; item_border: string; item_color: string }> = {
  primary: { item_bg: 'rgba(63,176,116,0.08)', item_border: '#2a5138', item_color: '#8fe0a8' },
  gold:    { item_bg: 'rgba(196,154,44,0.08)',  item_border: '#5a4820', item_color: '#E3B94A' },
  neutral: { item_bg: 'rgba(180,200,190,0.04)', item_border: '#1a2620', item_color: '#a7b1ab' },
}
...
function SeccionDark({ titulo, items, tone }: { titulo: string; items: Record<string, any>; tone: Tone }) {
  ...
  return (
    <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
      <h4 className="font-display font-semibold text-[13px] text-[#e8ece9] mb-3">{titulo}</h4>
