...
export default function PaginaReprocann() {
  ...
  const estadoBadge: Record<string, { color: string; bg: string; border: string }> = {
    presentado: { color: '#6bcf8e', bg: 'rgba(63,176,116,0.10)', border: '#2a5138' },
    observado:  { color: '#ff8a7a', bg: 'rgba(122,40,32,0.15)',   border: '#7a2820' },
    generado:   { color: '#a7b1ab', bg: 'rgba(180,200,190,0.05)', border: '#1a2620' },
  }
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <Shield className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">REPROCANN — Reporte Semestral</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              Res. ANMAT 1780/2025
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span>Decreto 27/2026</span>
            </div>
          </div>
      ...
      <div className="no-print px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
          <div className="flex flex-wrap items-end gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Año</p>
              <div className="relative">
                <select value={anio} onChange={e => setAnio(parseInt(e.target.value))}
                  className="appearance-none pl-3 pr-8 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] rounded-lg text-[12px] text-[#d3d8d4] outline-none cursor-pointer">
                  {[2024, 2025, 2026, 2027].map(y => <option key={y} value={y}>{y}</option>)}
                </select>
                <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#5a6560] pointer-events-none" />
              </div>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1.5">Semestre</p>
              <div className="relative">
            ...
            <button onClick={generar} disabled={cargando}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#3fb074]/15 hover:bg-[#3fb074]/25 border border-[#2a5138] text-[#8fe0a8] rounded-lg text-[12px] font-medium transition-colors disabled:opacity-50">
            ...
            {datos && (
              <>
                <button onClick={guardar} disabled={guardando}
                  className="flex items-center gap-1.5 px-4 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] text-[#a7b1ab] rounded-lg text-[12px] transition-colors disabled:opacity-50">
                  {guardando ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" strokeWidth={1.8} />}
                  Guardar
                </button>
                <button onClick={imprimir}
                  className="flex items-center gap-1.5 px-4 py-2 bg-[#C49A2C]/15 border border-[#5a4820] text-[#E3B94A] rounded-lg text-[12px] transition-colors">
        ...
        {msg && (
          <div className={`flex items-center gap-2 rounded-xl p-3 text-[12px] border ${
            msg.startsWith('Error')
              ...
              : 'bg-[#3fb074]/10 border-[#2a5138] text-[#8fe0a8]'
          }`}>
          ...
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Building2 className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
                <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">Datos del Titular (FIS S.A.S.)</h3>
              </div>
              {!orgEditando && (
                <button onClick={() => setOrgEditando(true)}
                  className="text-[11px] text-[#6bcf8e] hover:text-[#8fe0a8] font-medium">
                  Editar
                </button>
              )}
            </div>
            {!orgEditando ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="rounded-lg bg-[#111916] border border-[#1a2620] p-2.5">
                    <p className="text-[9px] uppercase tracking-[0.14em] text-[#5a6560] font-medium">{label}</p>
                    <p className={`text-[12px] mt-0.5 ${warn ? 'text-[#E3B94A] font-semibold' : 'text-[#d3d8d4]'}`}>
        ...
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <Paperclip className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">
                Documentos adjuntos <span className="text-[#5a6560] tabular-nums">({adjuntos.length})</span>
              </h3>
            </div>
            <label className="cursor-pointer flex items-center gap-1.5 px-3 py-1.5 bg-[#3fb074]/15 hover:bg-[#3fb074]/25 border border-[#2a5138] text-[#8fe0a8] rounded-lg text-[11px] font-medium transition-colors">
          ...
          <p className="text-[10px] text-[#5a6560] mb-2">Imagenes, PDFs, Excel, Word, ZIP. Signed URLs 1h.</p>
          {adjuntos.length === 0 ? (
            <p className="text-[11.5px] text-[#5a6560] text-center py-4">Sin archivos todavia</p>
          ) : (
            <ul className="divide-y divide-[#1a2620]">
              {adjuntos.map(a => (
                <li key={a.name} className="flex items-center gap-3 py-2 text-[11.5px]">
                  <Paperclip className="w-3 h-3 text-[#5a6560] flex-shrink-0" />
                  <a href={a.url} target="_blank" rel="noopener noreferrer"
                    className="font-mono text-[#6bcf8e] hover:text-[#8fe0a8] truncate flex-1">
                    {a.name.replace(/^\d+_/, '')}
                  </a>
                  <span className="text-[#5a6560] tabular-nums">{(a.size / 1024).toFixed(1)} KB</span>
                  <span className="text-[#5a6560] hidden sm:block tabular-nums">
                    {a.created_at ? new Date(a.created_at).toLocaleDateString('es-AR') : ''}
                  </span>
                  <button onClick={() => eliminarAdjunto(a.name)}
                    className="text-[#5a6560] hover:text-[#ff8a7a] flex-shrink-0">
        ...
        {datos && !orgRep?.reprocann && (
          <div className="flex items-center gap-2 rounded-xl p-3 text-[12px] bg-[#C49A2C]/10 border border-[#5a4820] text-[#E3B94A]">
        ...
        {sinDatos && (
          <div className="rounded-xl p-3 text-[12px] bg-[#0e1411] border border-[#1a2620] text-[#8e9892]">
        ...
        {historial.length > 0 && (
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
            <div className="px-4 py-3 border-b border-[#1a2620]">
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">Historial de reportes</h3>
            </div>
            <ul className="divide-y divide-[#1a2620]">
              {historial.map(h => {
                const b = estadoBadge[h.estado] || estadoBadge.generado
                return (
                  <li key={h.id} className="flex items-center justify-between gap-3 px-4 py-2.5 text-[11.5px] hover:bg-[#111916] transition-colors">
                    <span className="font-mono tabular-nums text-[#a7b1ab]">{h.periodo_anio}-S{h.periodo_semestre}</span>
                    <span className="text-[#5a6560] tabular-nums hidden sm:block">
                    ...
                    <span className="text-[#5a6560] font-mono">{h.numero_presentacion || '—'}</span>
                  </li>
                )
              })}
            </ul>
      ...
      {datos && (
        <div className="reprocann-print px-6 pb-10 space-y-6">
          <style>{`
            ...
            .reprocann-print h1 { font-family: 'Space Grotesk', 'Inter', sans-serif; font-size: 28px; font-weight: 800; text-align: center; margin: 0 0 8px 0; letter-spacing: -0.03em; background: linear-gradient(135deg, #10b981 0%, #059669 100%); -webkit-background-clip: text; background-clip: text; color: transparent; -webkit-text-fill-color: transparent; }
            .reprocann-print h2 { font-family: 'Space Grotesk', 'Inter', sans-serif; font-size: 15px; font-weight: 700; color: #10b981; border-bottom: 2px solid rgba(16,185,129,0.3); padding-bottom: 6px; margin: 28px 0 12px 0; letter-spacing: -0.01em; text-transform: uppercase; }
            .reprocann-print h3 { font-size: 13px; font-weight: 600; margin: 14px 0 8px 0; color: #86efac; letter-spacing: 0.05em; text-transform: uppercase; }
            ...
            .reprocann-print th { background: rgba(16,185,129,0.15); color: #86efac; font-weight: 600; font-size: 10px; text-transform: uppercase; letter-spacing: 0.04em; }
            ...
            .reprocann-print code, .reprocann-print .mono { font-family: 'JetBrains Mono', ui-monospace, monospace; font-weight: 500; font-size: 10.5px; letter-spacing: -0.01em; color: #10b981; }
            .reprocann-badge { display: inline-block; padding: 2px 10px; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; border-radius: 4px; background: rgba(16,185,129,0.2); color: #86efac; border: 1px solid rgba(16,185,129,0.4); }
          `}</style>
...
function FormOrg({ org, onCancel, onGuardar, guardando }: { org: any; onCancel: () => void; onGuardar: (d: any) => void; guardando: boolean }) {
  ...
  const input = (k: string, label: string, placeholder?: string) => (
    <label key={k} className="block">
      <span className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-1 block">{label}</span>
      <input type="text" value={f[k] || ''} onChange={e => setF({...f, [k]: e.target.value})}
        placeholder={placeholder}
        className="w-full px-3 py-2 bg-[#111916] border border-[#1a2620] hover:border-[#243429] focus:border-[#2a5138] rounded-lg text-[12px] text-[#d3d8d4] placeholder-[#5a6560] outline-none transition-colors" />
    </label>
  )
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={() => onGuardar(f)} disabled={guardando}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#3fb074]/15 border border-[#2a5138] text-[#8fe0a8] rounded-lg text-[11.5px] font-medium transition-colors disabled:opacity-50">
        ...
        <button onClick={onCancel} disabled={guardando}
          className="px-3 py-1.5 bg-[#111916] border border-[#1a2620] text-[#a7b1ab] rounded-lg text-[11.5px] transition-colors">
          Cancelar
        </button>
      </div>
    </div>
