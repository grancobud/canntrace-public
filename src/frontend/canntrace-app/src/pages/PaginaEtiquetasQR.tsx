...
<button class="no-print" onclick="window.print()" style="margin-top:1rem;padding:.5rem 1rem;background:#10b981;color:white;border:none;border-radius:4px;cursor:pointer">Imprimir</button>
...
<script>QRCode.toCanvas(document.getElementById('qr'),'${url}',{width:160})</script>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans pb-20">

      {/* TopBar inline */}
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <QrCode className="w-4 h-4 text-[#3fb074] shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Etiquetas QR</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">{lotes.length} lotes con código QR público para escanear</div>
          </div>
      ...
      <div className="p-4 sm:p-6 space-y-4">
        ...
        {/* Filtros */}
        <div className="flex flex-wrap gap-3 bg-[#0e1411] border border-[#1a2620] rounded-xl p-3">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-[#0a0f0d] border border-[#1a2620] rounded-lg text-[13px] text-[#d3d8d4] placeholder-[#5a6560] focus:outline-none focus:border-[#2a5138] transition-colors" />
          </div>
          <select value={filtroTipo} onChange={e => setFiltroTipo(e.target.value)}
            className="px-3 py-2 bg-[#0a0f0d] border border-[#1a2620] rounded-lg text-[13px] text-[#d3d8d4] focus:outline-none focus:border-[#2a5138] transition-colors">
        ...
        {cargando ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtrados.map(l => {
              ...
              return (
                <div key={l.id} className="bg-[#0e1411] border border-[#1a2620] hover:border-[#2a5138] rounded-xl p-4 space-y-3 transition-colors">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] uppercase text-[#5a6560] font-semibold tracking-wide">{l.productos?.tipo_producto?.replace(/_/g, ' ')}</p>
                      <p className="text-[13px] font-mono font-bold text-[#e8ece9] truncate tabular-nums">{l.codigo_lote}</p>
                      {l.datos_extra?.camada && <p className="text-[10px] text-[#8e9892] mt-0.5">Camada {l.datos_extra.camada}</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => copiar(url)}
                      className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-[#0a0f0d] border border-[#1a2620] hover:border-[#243429] text-[#8e9892] hover:text-[#d3d8d4] rounded-lg text-[11px] transition-colors">
                    ...
                    <button onClick={() => imprimir(l)}
                      className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-[#3fb074]/15 border border-[#2a5138] text-[#8fe0a8] hover:bg-[#3fb074]/25 rounded-lg text-[11px] transition-colors">
        ...
        {filtrados.length === 0 && !cargando && (
          <div className="text-center py-16 text-[#5a6560]">
      ...
      {verGS1 && (
        <Suspense fallback={<div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center"><Loader2 className="w-10 h-10 text-[#3fb074] animate-spin" /></div>}>
      ...
      {verQR && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4" onClick={() => setVerQR(null)}>
          <div className="bg-[#0e1411] border border-[#1a2620] rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="text-[10px] uppercase font-semibold text-[#5a6560] tracking-wide">Código QR público</p>
                <h3 className="text-[16px] font-bold text-[#e8ece9] font-mono tabular-nums mt-0.5">{verQR.codigo_lote}</h3>
                <p className="text-[11px] text-[#8e9892] mt-1">{verQR.productos?.nombre} · {verQR.datos_extra?.camada}</p>
              </div>
              <button onClick={() => setVerQR(null)} className="text-[#5a6560] hover:text-[#d3d8d4] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            {/* QR sobre fondo blanco — necesario para escaneo */}
            ...
            <p className="text-[11px] text-center text-[#5a6560] mt-3 font-mono break-all">{urlDe(verQR.codigo_lote)}</p>
            <div className="flex gap-2 mt-4">
              <button onClick={() => copiar(urlDe(verQR.codigo_lote))}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#0a0f0d] border border-[#1a2620] hover:border-[#243429] text-[#8e9892] hover:text-[#d3d8d4] rounded-xl text-[13px] font-medium transition-colors">
                <Copy className="w-4 h-4" /> {copiado ? 'Copiado!' : 'Copiar link'}
              </button>
              <button onClick={() => imprimir(verQR)}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#3fb074]/15 border border-[#2a5138] text-[#8fe0a8] hover:bg-[#3fb074]/25 rounded-xl text-[13px] font-medium transition-colors">
