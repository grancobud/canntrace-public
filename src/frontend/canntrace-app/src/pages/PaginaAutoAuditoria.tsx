...
export default function PaginaAutoAuditoria() {
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans print:bg-white ct-page-scroll" id="auditoria-pdf">
      {/* TopBar */}
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620] no-print">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <ClipboardCheck className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Auto-Auditoria del Sistema</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              {checks.length > 0
                ? <><span className="tabular-nums text-[#8fe0a8]">{porcentaje}%</span> cumplimiento · <span className="tabular-nums">{totalOk}</span> OK · <span className="tabular-nums text-[#E3B94A]">{totalAlerta}</span> alertas · <span className="tabular-nums text-[#ff8a7a]">{totalFallo}</span> fallos</>
                : 'Verificacion automatica GAMP5'}
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span> CUMCS IMC-GAP</span>
            </div>
          </div>
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10.5px] uppercase tracking-widest font-medium transition-colors ${
              mostrarCodigos
                ? 'border-[#3fb074]/30 bg-[#3fb074]/10 text-[#8fe0a8]'
                : 'border-[#1a2620] bg-transparent text-[#5a6560] hover:text-[#8fe0a8] hover:border-[#3fb074]/30'
            }`}
          ...
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-[#3fb074]/30 bg-[#3fb074]/10 text-[#8fe0a8] text-[10.5px] uppercase tracking-widest font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-[#6bcf8e] shadow-[0_0_6px_rgba(107,207,142,0.8)]" />GAMP5 Cat5
          </span>
        </div>
      </div>
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        {/* Score visual (web) */}
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-5 print-page-break no-print">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div>
              <h3 className="text-[13px] font-display font-semibold text-[#e8ece9]">Score de Cumplimiento</h3>
          ...
          <div className="flex items-center gap-6 flex-wrap">
            <div className="relative w-24 h-24">
              <svg className="w-24 h-24 -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="42" fill="none" stroke="#1a2620" strokeWidth="8" />
                <circle cx="50" cy="50" r="42" fill="none"
                  stroke={porcentaje >= 80 ? '#3fb074' : porcentaje >= 50 ? '#E3B94A' : '#ff6b5a'}
                  strokeWidth="8" strokeLinecap="round"
                  strokeDasharray={`${porcentaje * 2.64} 264`} />
              </svg>
              <span className="absolute inset-0 flex items-center justify-center text-[22px] font-bold font-display text-[#e8ece9] tabular-nums">{porcentaje}%</span>
            </div>
            <div className="flex gap-5">
              <div className="text-center"><p className="text-[22px] font-bold font-display text-[#3fb074] tabular-nums">{totalOk}</p><p className="text-[10px] text-[#5a6560] uppercase tracking-[0.12em]">Correctos</p></div>
              <div className="text-center"><p className="text-[22px] font-bold font-display text-[#E3B94A] tabular-nums">{totalAlerta}</p><p className="text-[10px] text-[#5a6560] uppercase tracking-[0.12em]">Alertas</p></div>
              <div className="text-center"><p className="text-[22px] font-bold font-display text-[#ff8a7a] tabular-nums">{totalFallo}</p><p className="text-[10px] text-[#5a6560] uppercase tracking-[0.12em]">Fallos</p></div>
              <div className="text-center"><p className="text-[22px] font-bold font-display text-[#747e78] tabular-nums">{checks.length}</p><p className="text-[10px] text-[#5a6560] uppercase tracking-[0.12em]">Total</p></div>
            </div>
          </div>
        </div>
        ...
        {ejecutando ? (
          <div className="text-center py-12 no-print">
            <Loader2 className="w-10 h-10 text-[#3fb074] animate-spin mx-auto" />
            <p className="mt-3 text-[12.5px] text-[#5a6560]">Ejecutando verificaciones...</p>
          </div>
        ) : (
          <div className="space-y-4">
            {categorias.map((cat, idx) => {
              ...
              return (
                <div key={cat} className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden print-block no-print">
                  <div className="px-4 py-3 bg-[#111916] border-b border-[#1a2620] flex items-center justify-between">
                    <h4 className="text-[12.5px] font-display font-semibold text-[#e8ece9]"><span className="print-show" style={{display:'inline'}}>2.{idx + 1}. </span>{cat}</h4>
                    <span className="text-[10.5px] text-[#5a6560] tabular-nums">{okCat}/{checksCategoria.length} OK</span>
                  </div>
                        ...
                        <div key={check.id} className="px-4 py-3 print-page-break">
                          <div className="flex items-start gap-3">
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                              check.resultado === 'ok' ? 'bg-[#3fb074]/15' :
                              check.resultado === 'alerta' ? 'bg-[#C49A2C]/15' : 'bg-[#7a2820]/30'
                            }`}>
                              {check.resultado === 'ok' ? <CheckCircle2 className="w-4 h-4 text-[#3fb074]" /> :
                              check.resultado === 'alerta' ? <AlertTriangle className="w-3.5 h-3.5 text-[#E3B94A]" /> :
                              <XCircle className="w-4 h-4 text-[#ff6b5a]" />}
                            </div>
                            {Icono && <Icono className="w-4 h-4 text-[#454e48] flex-shrink-0 mt-1 no-print" strokeWidth={1.75} />}
                            <div className="flex-1 min-w-0">
                              <p className="text-[12.5px] font-medium text-[#d3d8d4]">{check.nombre}</p>
                              <p className="text-[11px] text-[#5a6560] mt-0.5 leading-snug">{check.detalle}</p>
                            </div>
                            {mostrarCodigos && (
                              <span className="text-[10px] font-mono text-[#454e48] flex-shrink-0 mt-1">{check.referencia}</span>
                            )}
                          </div>
        ...
        <div className="mt-4 p-4 rounded-xl bg-[#3fb074]/8 border border-[#2a5138] text-[11px] text-[#6bcf8e] text-center no-print">
