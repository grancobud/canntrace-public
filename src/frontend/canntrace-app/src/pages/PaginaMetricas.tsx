...
const TabLoader = () => (
  <div className="text-center py-16">
    <Loader2 className="w-10 h-10 animate-spin text-[#3fb074] mx-auto" />
  </div>
)
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans pb-20">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <BarChart3 className="w-4 h-4 text-[#3fb074] shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Métricas</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">Análisis visual y comparativo por camada</div>
          </div>
      ...
      <div className="sticky top-[49px] sm:top-[53px] z-30 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-1.5 px-3 sm:px-6 py-2 flex-wrap">
          <div className="flex items-center gap-1 bg-[#0e1411] border border-[#1a2620] rounded-lg p-0.5">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-medium transition-all ${
                  tab === t.id
                    ? 'bg-[#3fb074]/15 border border-[#2a5138] text-[#8fe0a8]'
                    : 'text-[#8e9892] hover:text-[#d3d8d4] border border-transparent'
                }`}>
          ...
          {tab !== 'kpis' && tab !== 'heatmap' && (
            <div className="flex items-center gap-1.5 ml-2 flex-wrap">
              <span className="text-[10px] uppercase text-[#5a6560] font-semibold tracking-wide">Camada:</span>
              {CAMADAS.map(c => (
                <button key={c} onClick={() => setCamadaSel(c)}
                  className={`px-2 py-1 rounded-md text-[11px] font-bold font-mono tabular-nums transition-all ${
                    camadaSel === c
                      ? 'bg-[#C49A2C]/20 border border-[#5a4820] text-[#E3B94A]'
                      : 'bg-[#0e1411] border border-[#1a2620] text-[#8e9892] hover:text-[#d3d8d4] hover:border-[#243429]'
                  }`}>
                  {c}
                </button>
              ))}
      ...
      <div className="ct-dash-wrap p-4 sm:p-6 space-y-4">
        ...
        {tab === 'kpis' && metricas && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              className="bg-[#0e1411] border border-[#1a2620] rounded-xl p-4">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <p className="text-[13px] font-display font-semibold text-[#e8ece9]">Comparador por camada</p>
                <span className="text-[10px] uppercase tracking-wider font-medium px-2 py-0.5 rounded bg-[#3fb074]/10 border border-[#2a5138] text-[#8fe0a8] tabular-nums">{metricas.por_camada.length} camadas</span>
              </div>
              <BarChart
                ...
                colors={['#6bcf8e', '#3fb074', '#E3B94A', '#C49A2C']}
                yAxisWidth={42}
                showAnimation
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              className="bg-[#0e1411] border border-[#1a2620] rounded-xl p-4">
              <p className="text-[13px] font-display font-semibold text-[#e8ece9] mb-3">Detalle por camada</p>
              <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-3"
                ...
                {metricas.por_camada.map(c => {
                  ...
                  const borderLeft = isRDWC ? 'border-l-[3px] border-l-[#3fb074]' : 'border-l-[3px] border-l-[#C49A2C]'
                  const sistemaBg = isRDWC
                    ? 'bg-[#3fb074]/10 border border-[#2a5138] text-[#8fe0a8]'
                    : 'bg-[#C49A2C]/10 border border-[#5a4820] text-[#E3B94A]'
                  return (
                    <motion.div key={c.camada}
                      variants={{ hidden: { opacity: 0, y: 8 }, visible: { opacity: 1, y: 0, transition: { duration: 0.32, ease: EASE } } }}
                      className={`${borderLeft} border border-[#1a2620] hover:border-[#2a5138] rounded-xl p-4 bg-[#0a0f0d] transition-colors`}>
                      <div className="flex items-center justify-between mb-3 gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[15px] font-display font-bold text-[#e8ece9] tabular-nums">Camada {c.camada}</span>
                        </div>
                        <div className="text-[11px] text-[#5a6560] font-mono tabular-nums shrink-0">
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              className="bg-[#0e1411] border border-[#1a2620] rounded-xl p-4">
              <p className="text-[13px] font-display font-semibold text-[#e8ece9] mb-3">Mermas por camada (%)</p>
              <AreaChart
                data={metricas.por_camada.map(c => ({
                  camada: c.camada,
        ...
        {tab === 'timeline' && (
          <Suspense fallback={<TabLoader />}>
            {cargando ? <TabLoader /> :
             timeline.length === 0
               ? <p className="text-center py-12 text-[#5a6560]">Sin eventos para {camadaSel}</p>
...
function KpiCard({ label, valor, sub, tone }: { label: string; valor: any; sub: string; tone: 'primary' | 'gold' | 'warn' }) {
  const accent = tone === 'gold' ? 'border-l-[#C49A2C]' : tone === 'warn' ? 'border-l-[#E3B94A]' : 'border-l-[#3fb074]'
  const valColor = tone === 'gold' ? 'text-[#E3B94A]' : tone === 'warn' ? 'text-[#E3B94A]' : 'text-[#8fe0a8]'
  return (
    <div className={`bg-[#0e1411] border border-[#1a2620] border-l-[3px] ${accent} rounded-xl p-4`}>
      <p className="text-[10px] uppercase tracking-wide font-semibold text-[#5a6560] mb-1">{label}</p>
      <p className={`text-2xl font-display font-bold font-mono tabular-nums ${valColor}`}>{valor}</p>
      <p className="text-[11px] text-[#8e9892] mt-1">{sub}</p>
    </div>
  )
}
...
function MiniStat({ label, val }: { label: string; val: any }) {
  return (
    <div className="bg-[#111916] border border-[#1a2620] rounded-lg p-2">
      <p className="text-[9px] uppercase text-[#5a6560] font-semibold tracking-wide">{label}</p>
      <p className="text-[12px] font-bold font-mono tabular-nums text-[#d3d8d4]">{val}</p>
    </div>
  )
}
...
function SmoothMermaBar({ label, pct, left, right, warn, danger }: { label: string; pct: number; left: string; right: string; warn?: boolean; danger?: boolean }) {
  const fill = danger
    ...
    : warn
      ? 'linear-gradient(90deg, #C49A2C 0%, #E3B94A 100%)'
      : 'linear-gradient(90deg, #3fb074 0%, #6bcf8e 100%)'
  const txt = danger ? '#ff8a7a' : warn ? '#E3B94A' : '#8fe0a8'
  ...
  return (
    <div>
      <div className="flex justify-between items-baseline mb-1">
        ...
        <span className="text-[10.5px] text-[#5a6560] font-mono tabular-nums">{left} → {right}</span>
      </div>
      <div className="h-1.5 rounded-full bg-[#162420] overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
