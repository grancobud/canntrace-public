...
function colorVals(v: number, min: number, max: number): { text: string; bg: string; border: string } {
  ...
  return { text: '#8fe0a8', bg: 'rgba(63,176,116,0.10)', border: '#2a5138' }
}
    ...
    <div className="flex-1 overflow-y-auto bg-[#0a0f0d] text-[#d3d8d4] font-sans ct-page-scroll">
      ...
      <div className="sticky top-0 z-40 bg-[#0a0f0d]/95 backdrop-blur-[2px] border-b border-[#1a2620]">
        <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-6 py-3">
          <Calculator className="w-4 h-4 text-[#3fb074] flex-shrink-0" strokeWidth={1.8} />
          <div className="min-w-0">
            <h1 className="font-display font-bold tracking-tight text-[15px] sm:text-[17px] text-[#e8ece9]">Calculadoras de Cultivo</h1>
            <div className="mt-0.5 text-[10.5px] sm:text-[11px] text-[#5a6560]">
              VPD + DLI
              <span className="hidden sm:inline"><span className="text-[#343c37] mx-1">│</span>Optimos por fase y sistema RDWC/COCO</span>
            </div>
          </div>
      ...
      <div className="px-3 sm:px-6 py-4 sm:py-5 pb-20 space-y-4 sm:space-y-5">
        ...
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4 space-y-3">
          {/* Fases */}
          <div>
            <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-2">Fase</p>
                ...
                <button key={f.id} onClick={() => setFase(f.id)}
                  className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors ${
                    fase === f.id
                      ...
                      : 'bg-[#111916] border border-[#1a2620] text-[#8e9892] hover:border-[#243429] hover:text-[#d3d8d4]'
                  }`}>
                  {f.label}
                </button>
              ))}
          ...
          {/* Sistemas */}
          <div>
            <p className="text-[10px] uppercase tracking-[0.14em] text-[#5a6560] font-medium mb-2">Sistema</p>
            <div className="flex flex-wrap gap-1.5">
              {SISTEMAS.map(s => (
                  ...
                  className={`px-2.5 py-1 rounded-md text-[11px] font-bold transition-colors ${
                    sistema === s.id
                      ? s.id === 'RDWC'
                        ...
                        : 'bg-[#C49A2C]/15 border border-[#5a4820] text-[#E3B94A]'
                      : 'bg-[#111916] border border-[#1a2620] text-[#8e9892] hover:border-[#243429] hover:text-[#d3d8d4]'
                  }`}>
                  {s.label}
                </button>
          ...
          {/* Nota sistema */}
          <div className="flex items-start gap-2 rounded-lg bg-[#111916] border border-[#1a2620] px-3 py-2">
            <Info className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-[#3fb074]" strokeWidth={1.8} />
            <p className="text-[10.5px] text-[#747e78]">{sisSel.note}</p>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">VPD · Vapor Pressure Deficit</h3>
            </div>
            <div className="space-y-3">
              <label className="block">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-[#8e9892] flex items-center gap-1">
                    <Thermometer className="w-3 h-3" strokeWidth={1.8} />
                    Temperatura: <b className="text-[#e8ece9] font-mono ml-1">{temp}°C</b>
                  </span>
                  {(() => {
                ...
                <input type="range" min="15" max="35" step="0.5" value={temp}
                  onChange={e => setTemp(+e.target.value)} className="w-full accent-[#3fb074]" />
              </label>
              <label className="block">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-[#8e9892] flex items-center gap-1">
                    <Droplets className="w-3 h-3" strokeWidth={1.8} />
                    Humedad: <b className="text-[#e8ece9] font-mono ml-1">{rh}%</b>
                  </span>
                  {(() => {
                ...
                <input type="range" min="20" max="95" step="1" value={rh}
                  onChange={e => setRh(+e.target.value)} className="w-full accent-[#3fb074]" />
              </label>
            </div>
            {/* Resultados VPD */}
            ...
            <p className="text-[10px] text-[#5a6560] mt-3 tabular-nums">
              Rango optimo: <b className="text-[#6bcf8e]">{faseSel.vpd[0]}-{faseSel.vpd[1]} kPa</b>
            </p>
          </div>
          ...
          <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] p-4">
            <div className="flex items-center gap-2 mb-4">
              <Sun className="w-4 h-4 text-[#E3B94A]" strokeWidth={1.8} />
              <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">DLI · Daily Light Integral</h3>
            </div>
            <div className="space-y-3">
              <label className="block">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-[#8e9892]">PPFD: <b className="text-[#e8ece9] font-mono ml-1">{ppfd}</b> µmol/m²/s</span>
                </div>
                <input type="range" min="100" max="1500" step="10" value={ppfd}
                  onChange={e => setPpfd(+e.target.value)} className="w-full accent-[#C49A2C]" />
              </label>
              <label className="block">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-[#8e9892]">Fotoperíodo: <b className="text-[#e8ece9] font-mono ml-1">{horas}</b> hs</span>
                </div>
                <input type="range" min="0" max="24" step="1" value={horas}
                  onChange={e => setHoras(+e.target.value)} className="w-full accent-[#C49A2C]" />
              </label>
            </div>
            ...
            <p className="text-[10px] text-[#5a6560] mt-3 tabular-nums">
              Rango optimo: <b className="text-[#6bcf8e]">{faseSel.dli[0]}-{faseSel.dli[1] || '—'} mol/m²/día</b>
            </p>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-[#0e1411] border border-[#1a2620] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#1a2620] flex items-center gap-2">
            <Leaf className="w-3.5 h-3.5 text-[#6bcf8e]" strokeWidth={1.8} />
            <h3 className="font-display font-semibold text-[13px] text-[#e8ece9]">Tabla de rangos óptimos (PETE HOPE · Ka)</h3>
          </div>
              ...
              <thead>
                <tr className="border-b border-[#1a2620]">
                  {['Fase','Temp (°C)','RH (%)','VPD (kPa)','DLI (mol/m²/d)'].map(h => (
                    <th key={h} className="px-3 py-2.5 text-left text-[10px] uppercase tracking-[0.12em] text-[#5a6560] font-medium">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {FASES.map(f => (
                  <tr key={f.id} className={`border-b border-[#1a2620] last:border-0 transition-colors ${f.id === fase ? 'bg-[#3fb074]/08' : 'hover:bg-[#111916]'}`}>
                    <td className="px-3 py-2 font-medium text-[#d3d8d4]">{f.label}</td>
                    <td className="px-3 py-2 text-[#a7b1ab] font-mono tabular-nums">{f.temp[0]}-{f.temp[1]}</td>
                    <td className="px-3 py-2 text-[#a7b1ab] font-mono tabular-nums">{f.rh[0]}-{f.rh[1]}</td>
                    <td className="px-3 py-2 text-[#a7b1ab] font-mono tabular-nums">{f.vpd[0]}-{f.vpd[1]}</td>
                    <td className="px-3 py-2 text-[#a7b1ab] font-mono tabular-nums">{f.dli[0]}-{f.dli[1] || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="px-4 py-2.5 text-[10px] text-[#5a6560] border-t border-[#1a2620]">
