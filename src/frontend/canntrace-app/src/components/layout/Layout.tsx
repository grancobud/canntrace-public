...
import { useGlobalCumcsRealtimeInvalidation } from '../../hooks/useRealtimeInvalidation'
...
export default function Layout() {
  ...
  // Cadena reactiva P2.4: cualquier INSERT/UPDATE/DELETE en las 13 tablas clave
  // (operaciones, lotes, registros_*) invalida los queries de TanStack Query
  // correspondientes — toda la UI se actualiza sin F5.
  useGlobalCumcsRealtimeInvalidation()
      ...
      <div className="flex min-h-screen bg-primary-100 dark:bg-surface-950">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden pb-14">
          <div className="flex items-center gap-3 px-4 py-3 bg-surface-900 text-white">
            ...
            <span className="text-lg font-bold">CannTrace</span>
          </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#0a0f0d]">
      ...
      <style>{`
        ...
        .ct-resizer { position: relative; width: 10px; flex-shrink: 0; cursor: col-resize; user-select: none; touch-action: none; z-index: 30; background: #0a0f0d; }
        .ct-resizer-line { position: absolute; top: 0; bottom: 0; left: 50%; transform: translateX(-50%); width: 1px; background: #111916; transition: background 160ms ease, width 160ms ease; }
        .ct-resizer:hover .ct-resizer-line, .ct-resizer.is-dragging .ct-resizer-line { background: #3fb074; width: 2px; }
        .ct-resizer-grip { position: absolute; top: 50%; left: 50%; transform: translate(-50%,-50%); width: 4px; height: 40px; border-radius: 4px; background: #243429; opacity: 0; transition: opacity 160ms ease, background 160ms ease, transform 160ms ease; pointer-events: none; }
        .ct-resizer:hover .ct-resizer-grip, .ct-resizer.is-dragging .ct-resizer-grip { opacity: 1; background: #6bcf8e; transform: translate(-50%,-50%) scaleY(1.15); }
        .ct-resizer-tooltip { position: absolute; top: 50%; left: calc(50% + 14px); transform: translateY(-50%); white-space: nowrap; background: #0e1411; border: 1px solid #1a2620; color: #d3d8d4; font-size: 11.5px; padding: 6px 9px; border-radius: 6px; opacity: 0; pointer-events: none; transition: opacity 180ms ease 120ms; box-shadow: 0 6px 24px rgba(0,0,0,.4); line-height: 1.35; }
        .ct-resizer-tooltip .kbd { display: inline-flex; align-items: center; margin-left: 8px; padding: 1px 5px; border-radius: 3px; background: #1a2620; border: 1px solid #243429; font-family: 'JetBrains Mono', ui-monospace, monospace; font-size: 10px; color: #8e9892; }
      ...
      `}</style>
      <aside data-ct-sidebar data-dragging={drag} style={{ width }} className="bg-[#0a0f0d] flex-shrink-0 overflow-hidden" aria-label="Navegacion principal">
